...
public class ReadWriteLockService {
    ...
    /**
     * 重写getDetail方法,内部全走批量接口,解决oom和超时
     * @return
     */
    public Map<Long,ReadWriteLockVO> getBatchDetail(Long projectId, List<Long> relationIds, ReadWriteLockType lockType, Long userId, Map<Long, BatchScript> batchScriptMap) {
        List<ReadWriteLock> readWriteLockList = new ArrayList<>(readWriteLockDao.getLocksByIds(projectId, lockType.name(), relationIds));
        Map<Long,ReadWriteLock> idAndBeanMap = new HashMap<>();
        readWriteLockList.forEach(bean->{
            idAndBeanMap.put(bean.getRelationId(),bean);
        });
        List<Long> lockedRelationIds = readWriteLockList.stream().map(ReadWriteLock::getRelationId).collect(Collectors.toList());
        // 逐个重写, 筛选出没有readWriteLock的relationId,建立新的readWriteLockVO出来
        List<ReadWriteLockVO> returnVOs = new ArrayList<>(relationIds.size());
        // 建立一个userId和userName的映射,避免多次重复调用userService.getUserName()
        HashMap<Long, String> userMap = new HashMap<>(32);
        Map<Long,ReadWriteLockVO> lockVOMap = new HashMap<>();
        // 按顺序存储一遍readWriteLockVO
        for (int idx = 0; idx < relationIds.size(); idx++) {
            ReadWriteLockVO readWriteLockVO ;
            if (!lockedRelationIds.contains(relationIds.get(idx))) {
                readWriteLockVO = new ReadWriteLockVO();
                readWriteLockVO.setLastKeepLockUserName(this.getUserNameMapCached(batchScriptMap.get(relationIds.get(idx)).getCreateUserId(), userMap));
                readWriteLockVO.setIsGetLock(false);
                readWriteLockVO.setGmtModified(batchScriptMap.get(relationIds.get(idx)).getGmtModified());
            } else {
                readWriteLockVO= ReadWriteLockVO.toVO(idAndBeanMap.get(relationIds.get(idx)));
                this.isGetLock(readWriteLockVO, userId);
                readWriteLockVO.setLastKeepLockUserName(this.getUserNameMapCached(readWriteLockVO.getModifyUserId(), userMap));
                returnVOs.add(readWriteLockVO);
            }
            lockVOMap.put(relationIds.get(idx),readWriteLockVO);
        }
        return lockVOMap;
    }
