package com.dtstack.batch.sql.hive.node;

import com.dtstack.batch.sql.Column;
import com.dtstack.batch.sql.Table;
import com.dtstack.batch.sql.hive.until.ASTNodeFunctionEnum;
import com.dtstack.batch.sql.node.BasicCall;
import com.dtstack.batch.sql.node.Identifier;
import com.dtstack.batch.sql.node.JoinCall;
import com.dtstack.batch.sql.node.LiteralIdentifier;
import com.dtstack.batch.sql.node.Node;
    ...
    private Node getNodeByparsetLateRelView(ASTNode astNode, Map<String, List<Identifier>> lateralView, String defultDb, Map<String, List<Column>> tableColumnsMap, Map<String, String> aliasToTable) {
        ...
        for (org.apache.hadoop.hive.ql.lib.Node node : children) {
            if (((ASTNode) node).getType() == HiveParser.TOK_SELECT) {
                getLateralViewParserResult((ASTNode) node, lateralView, defultDb, tableColumnsMap);
            } else if (((ASTNode) node).getType() == HiveParser.TOK_LATERAL_VIEW) {
                selectNode = getNodeByparsetLateRelView((ASTNode) node, lateralView, defultDb, tableColumnsMap, aliasToTable);
            } else if (((ASTNode) node).getType() == HiveParser.TOK_TABREF) {
