 ...
 * 下载TIDB的数据
 */
public class TiDBDownload implements IDownload {

    private static Logger logger = LoggerFactory.getLogger(TiDBDownload.class);

    private IDownloader pluginDownloader;

    public TiDBDownload(String sql, Long dtuicTenantId, String schema) throws Exception {
        JdbcInfo jdbcInfo = Engine2DTOService.getJdbcInfo(dtuicTenantId, null, EJobType.TIDB_SQL);
        ISourceDTO iSourceDTO = Engine2DTOService.get(dtuicTenantId, null, DataSourceType.TiDB.getVal(), schema, jdbcInfo);
    ...
    public String getFileName() {
        try {
            ...
            logger.error(String.format("获取getFileName失败,原因是%s", e.getMessage()));
        }
        return "";
    }
