 ...
 */
@Service
public class BatchTiDBTablePublishService implements ITablePublishService {
    ...
    public Integer publish(BatchTableInfo sourceTableInfo, Project sourceProject, Long sourceDtUicTenantId, Tenant produceTenant, Long userId) throws Exception {
        ...
        ProjectEngine projectEngine = projectEngineService.getProjectDb(sourceProject.getId(), MultiEngineType.TIDB.getType());
        Preconditions.checkNotNull(projectEngine, String.format("project %d not support type %s", sourceProject.getId(), MultiEngineType.TIDB.getType()));
        ...
        BatchTableInfo produceTable = batchTableInfoDao.getByTableName(sourceTableInfo.getTableName(),
                sourceProject.getTenantId(), sourceProject.getProduceProjectId(), ETableType.TIDB.getType());
        if (produceTable == null) {
            ProjectEngine produceProjectEngine = projectEngineService.getProjectDb(sourceProject.getProduceProjectId(), MultiEngineType.TIDB.getType());
            Preconditions.checkNotNull(produceProjectEngine, String.format("project %d not support type %s", sourceProject.getProduceProjectId(), MultiEngineType.TIDB.getType()));
            jdbcServiceImpl.executeQueryWithoutResult(produceTenant.getDtuicTenantId(), dtuicUserId, EJobType.TIDB_SQL, produceProjectEngine.getEngineIdentity(), createSql);
            return 1;
        } else {
            return 0;
        }
