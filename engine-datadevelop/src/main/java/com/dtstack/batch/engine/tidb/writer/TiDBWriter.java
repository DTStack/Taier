package com.dtstack.batch.engine.tidb.writer;
...
public class TiDBWriter {
	...
	private static final Logger logger = LoggerFactory.getLogger(TiDBWriter.class);
	...
	private static int writeTemplate(String fileDirPath, String fromLineDelimiter, String oriCharSet, int startLine,
		...
		try {
			...
			throw new RdosDefineException(String.format("第%s行数据异常,请检查，错误原因是：%s", currLineNum - (topLineIsTitle ? 1 : 0), e.getMessage()),
