...
public class BatchTaskService {
    ...
    public static Logger logger = LoggerFactory.getLogger(com.dtstack.batch.service.task.impl.BatchTaskService.class);

    @Autowired
    private JobParamReplace jobParamReplace;

    @Autowired
    private com.dtstack.batch.service.task.impl.BatchTaskParamShadeService batchTaskParamShadeService;
    ...
    @Autowired
    private com.dtstack.batch.service.task.impl.BatchTaskResourceService batchTaskResourceService;
    ...
    @Autowired
    private com.dtstack.batch.service.task.impl.BatchTaskResourceShadeService batchTaskResourceShadeService;
    ...
    @Autowired
    private com.dtstack.batch.service.task.impl.ReadWriteLockService readWriteLockService;
    ...
    public void init() {
        ...
        com.dtstack.batch.service.task.impl.BatchTaskService.jobSupportTypeMap.put(EDeployType.YARN.getType(), yarnSupportType);
        com.dtstack.batch.service.task.impl.BatchTaskService.jobSupportTypeMap.put(EDeployType.LIBRA.getType(), libraSupportType);
    }
        ...
        if (EJobType.SYNC.getVal().equals(distTask.getTaskType())) {
            ...
            if (Objects.nonNull(scrDirtyTable) && 1 == scrDirtyTable.getIsDirtyDataTable()) {
                ...
                if (Objects.nonNull(distDirtyTable)) {
                    com.dtstack.batch.service.task.impl.BatchTaskService.logger.info("copy src task {} dirty table  {} has exist ", srcTask.getId(), distDirtyTable);
                    return;
                }
                //需要创建脏数据表
    ...
    public BatchTaskBatchVO getTaskById(final ScheduleTaskVO ScheduleTaskVO) {
        ...
        if (task.getTaskType().intValue() == EJobType.SYNC.getVal().intValue()) {
            try {
                ...
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.error(String.format("同步任务json解析失败 taskId = {%s},错误= {%s}",task.getId(),e.getMessage()));
            ...
            for (final BatchTaskVersionDetail taskVersion : taskVO.getTaskVersions()) {
                try {
                    ...
                    com.dtstack.batch.service.task.impl.BatchTaskService.logger.error(String.format("同步任务json解析失败 taskVersionId = {%s},错误= {%s}",taskVersion.getId(),e.getMessage()));
        ...
        if (task.getTaskType().equals(EJobType.PYTHON.getVal())
            ...
            if (StringUtils.isBlank(task.getExeArgs())) {
                ...
                try {
                    ...
                    exeArgsJson.put(com.dtstack.batch.service.task.impl.BatchTaskService.CMD_OPTS, task.getExeArgs());
                    exeArgsJson.put(com.dtstack.batch.service.task.impl.BatchTaskService.OPERATE_MODEL, TaskOperateType.RESOURCE.getType());
                }
                if (task.getTaskType().equals(EJobType.PYTHON.getVal())) {
                ...
                taskVO.setOptions(exeArgsJson.getString(com.dtstack.batch.service.task.impl.BatchTaskService.CMD_OPTS));
                taskVO.setOperateModel(exeArgsJson.getIntValue(com.dtstack.batch.service.task.impl.BatchTaskService.OPERATE_MODEL));
            }
        }
    }
    ...
    public void parseTableFromSqlTask(final String currentDb, String sqlText, final List<Table> tables, final Integer taskType, final boolean isContainMainTable, Long projectId) {
        ...
        for (final String sql : sqls) {
            ...
            try {
                ...
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.warn("parse tables from sql error: ", e);
            }
        }
    }
    ...
    private ScheduleTaskShadeDTO buildScheduleTaskShadeDTO(final BatchTask batchTask, List<BatchTaskTask> allTaskTaskList) {
        ...
        // 设置项目的调度状态
        scheduleTaskShadeDTO.setProjectScheduleStatus(projectService.getProjectById(batchTask.getProjectId()).getScheduleStatus());
    ...
    private void checkFillPassword(final TaskResourceParam param) {
        ...
        if (param.getId() > 0 && EJobType.SYNC.getVal().equals(param.getTaskType())) {
            ...
            if (matcher.find()) {
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.debug("当前上送信息存在隐藏密码字段，准备执行旧密码回填操作");
    ...
    private Integer checkBeforeUpdateTask(TaskResourceParam param) {
        ...
        if (EJobType.SPARK.getVal().equals(param.getTaskType()) ||
            ...
            exeArgs.put(com.dtstack.batch.service.task.impl.BatchTaskService.CMD_OPTS, param.getOptions());
    ...
    private Integer operateSyncTask(TaskResourceParam param) {
        ...
        com.dtstack.batch.service.task.impl.BatchTaskService.logger.info("addOrUpdateTask with createModel {}", param.getCreateModel());
        ...
        if (param.getId() != 0 && !param.getIsEditBaseInfo()) {
            ...
            try {
                ...
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("check permission :" + e.getMessage());
    ...
    public BatchTaskBatchVO updateTask(final BatchTaskBatchVO task, final Boolean isEditBaseInfo) {
        ...
        if (task.getId() > 0) {//update
            ...
            com.dtstack.batch.service.task.impl.BatchTaskService.logger.info("success update batchTask, taskId:{}", task.getId());
            ...
            ReadWriteLockVO readWriteLockVO = this.readWriteLockService.getLock(
                    ...
                    null, null);
            task.setReadWriteLockVO(readWriteLockVO);
            com.dtstack.batch.service.task.impl.BatchTaskService.logger.info("success insert batchTask, taskId:{}", task.getId());
        }
    ...
    private boolean initTaskInfo(BatchTaskBatchVO task) {
        ...
        if (StringUtils.isBlank(task.getScheduleConf())) {
            task.setScheduleConf(com.dtstack.batch.service.task.impl.BatchTaskService.DEFAULT_SCHEDULE_CONF);
        } else {
        ...
        task.setSubmitStatus(ESubmitStatus.UNSUBMIT.getStatus());
        task.setPeriodType(com.dtstack.batch.service.task.impl.BatchTaskService.DEFAULT_SCHEDULE_PERIOD);
        String scConf = com.dtstack.batch.service.task.impl.BatchTaskService.DEFAULT_SCHEDULE_CONF;
        int period = com.dtstack.batch.service.task.impl.BatchTaskService.DEFAULT_SCHEDULE_PERIOD;
        if (task.getFlowId() != null && task.getFlowId() > 0) {
            final BatchTask flow = this.batchTaskDao.getOne(task.getFlowId());
            if (flow != null) {
    ...
    private void parseCreateTaskExeArgs(final ScheduleTaskVO task) {
        if (null != task && StringUtils.isNotBlank(task.getExeArgs())) {
            try {
                ...
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("parse exeArgs error {} ", task.getExeArgs(), e);
            }
        }
    }
    ...
    private Integer getLockVersion(final BatchTaskBatchVO task) {
        ...
        if (task.getReadWriteLockVO() == null) {
            ...
            if (lock.getModifyUserId().equals(task.getUserId())) {
                ...
                lockVersion = com.dtstack.batch.service.task.impl.BatchTaskService.INIT_LOCK_VERSION;
            }
        } else {
    ...
    private void formatSqlText(final ScheduleTaskVO taskVO, final JSONObject obj) {
        ...
        if (obj.get("job") != null && Constant.CREATE_MODEL_GUIDE == taskVO.getCreateModel()) {
            ...
            try {
                map = (Map<String, String>) com.dtstack.batch.service.task.impl.BatchTaskService.objectMapper.readValue(String.valueOf(obj.get("job")), Object.class);
                sqlText = JsonUtils.formatJSON(map);
            } catch (final IOException e) {
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("sqlText的json格式化失败{}" + e);
    ...
    private String doGetDefaultTaskParam(Long dtuicTenantId, final int engineType, final int computeType) {
        ...
        if (engineParamTmplByComputeType != null) {
            ...
            if (data == null) {
                com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("systbatchJobem don't have init param of engineType {} of compute Type: {} ,data {}", engineType, computeType, JSON.toJSONString(engineParamTmplByComputeType));
    ...
    public Long deleteTask(Long taskId, Long projectId, Long userId, Long tenantId, String sqlText) {
        ...
        if (StringUtils.isNotBlank(sqlText)) {
            ...
            com.dtstack.batch.service.task.impl.BatchTaskService.logger.info("sqlText 修改成功");
        } else {
            com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("deleteTask sqlText is null");
        }
        //删除任务
    ...
    public void checkName(String name, String type, Integer pid, Integer isFile, Long projectId) {
        if (StringUtils.isNotEmpty(name)) {
            if (!isFile.equals(com.dtstack.batch.service.task.impl.BatchTaskService.IS_FILE)) {
    ...
    public Map<String, Integer> countTask(final Long projectId, final Long tenantId) {
        ...
        final Integer submitCount = this.batchTaskDao.countByProjectIdAndSubmit(com.dtstack.batch.service.task.impl.BatchTaskService.IS_SUBMIT, projectId, tenantId);
        final BatchTaskDTO batchTaskDTO = new BatchTaskDTO();
    ...
    public Integer maxTaskVersion(final Long taskId) {
        ...
        if (maxVersionId == null) {
            com.dtstack.batch.service.task.impl.BatchTaskService.logger.error("maxVersion cannot be null, taskId={}", taskId) ;
     ...
     * @param taskTypes
     */
    public String againPublishTask(List<Integer> taskTypes) {
        return againPublishTask(taskTypes, null);
    }

    /**
     * 提交特定类型的任务，根据 filterMode 过滤任务信息
     *
     * @param taskTypes
     * @param filterMode
     * @return
     */
    public String againPublishTask(List<Integer> taskTypes, Integer filterMode) {
        List<BatchTask> batchTasks = listAllSubmitTask(com.dtstack.batch.enums.ESubmitStatus.SUBMIT.getStatus(), taskTypes);
        // 过滤任务信息
        batchTasks = filterTask(batchTasks, 1);
        ...
        CountDownLatch countDownLatch = new CountDownLatch(batchTasks.size());
        StringBuffer errorTaskMsg = new StringBuffer();
        for (BatchTask batchTask : batchTasks) {
            // 校验任务租户和项目是否仍然存在
            Tenant tenant = dtUicTenantIdMap.get(batchTask.getTenantId());
            Project project = projectMap.get(batchTask.getProjectId());
            // 项目或者租户为空，则说明该任务已无效，直接跳过
            ...
            // 离线数据库最新一次提交的版本信息
            BatchTaskVersion taskVersion = batchTaskVersionDao.getByTaskIdAndVersion(batchTask.getId(), batchTask.getVersion());
            // 如果不存在版本信息，但是引擎存在，则说明这部分任务保存之后未提交过，则需要运维手动处理
            if (taskVersion == null) {
                errorTaskMsg.append(String.format(LOGGER_TENANT_PROJECT_TASK_ERROR, tenant.getTenantName(), project.getProjectName(), batchTask.getName()));
                countDownLatch.countDown();
                continue;
            }

            // 如果引擎的版本与离线版本不同，则说明为脏数据，需要运维手动处理
            if (scheduleTaskShade.getVersionId() != taskVersion.getId().intValue()) {
                errorTaskMsg.append(String.format(LOGGER_TENANT_PROJECT_TASK_ERROR, tenant.getTenantName(), project.getProjectName(), batchTask.getName()));
                countDownLatch.countDown();
                continue;
            }

            TASK_PUBLISH_JOB.submit(() -> {
                try {
                    publishTask(batchTask.getProjectId(), batchTask.getId(), batchTask.getOwnerUserId(), "自定义函数数据处理自动提交", YES_IS_ROOT, IGNORE_CHECK, tenant.getDtuicTenantId(), null);
                    logger.info("提交成功的任务：" + String.format(LOGGER_AGAIN_PUSH_TASK, batchTask.getId(), batchTask.getName()));
                } catch (Exception e) {
                    logger.error(e.getMessage());
                    logger.error("提交失败的任务：" + String.format(LOGGER_AGAIN_PUSH_TASK, batchTask.getId(), batchTask.getName()));
                } finally {
                    countDownLatch.countDown();
                }
            });
        ...
        try {
            ...
            logger.error("批量提交任务失败 : ", e.getMessage(), e);
        }

        return errorTaskMsg.length() == 0 ? "Success" : errorTaskMsg.toString();
    }
     ...
     * @param filterMode
     */
    public List<BatchTask> filterTask(List<BatchTask> batchTasks, Integer filterMode) {
        if (CollectionUtils.isEmpty(batchTasks)) {
            return Lists.newArrayList();
        }

        // 目前为空，说明过滤非 SparkSQL 和 HiveSQL 任务，且过滤不存在自定义函数的任务
        if (filterMode == null || filterMode == 1) {
            return batchTasks.stream()
                    .filter(task -> EJobType.SPARK_SQL.getVal().equals(task.getTaskType()) || EJobType.HIVE_SQL.getVal().equals(task.getTaskType()))
                    .filter(task -> batchFunctionService.validContainSelfFunction(task.getSqlText(), task.getProjectId(), FunctionType.USER.getType()))
                    .collect(Collectors.toList());
        }
    ...
    /**
     * 查询所有已提交的Spark sql和Hive sql任务
     *
     * @param submitStatus
     * @param taskType
     * @return
     */
    public List<BatchTask> listAllSubmitTask(Integer submitStatus, List<Integer> taskType) {
        return batchTaskDao.listAllSubmitTask(submitStatus, taskType);
    }
