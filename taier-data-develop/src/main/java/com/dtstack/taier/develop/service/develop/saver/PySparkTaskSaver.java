 ...
 */
@Component
public class PySparkTaskSaver extends AbstractTaskSaver {
    ...
    public static Logger LOGGER = LoggerFactory.getLogger(PySparkTaskSaver.class);
    ...
    public void afterProcessing(TaskResourceParam taskResourceParam, TaskVO taskVO) {
        ...
        developTaskResourceService.save(taskVO, taskResourceParam.getResourceIdList(),  ResourceType.PYTHON.getType());
    }


    @Override
    public String processScheduleRunSqlText(Task task) {
        if (TaskCreateModelType.TEMPLATE.getType().equals(task.getCreateModel())) {
            String hdfsPath = uploadSqlText(task.getTenantId(), task.getSqlText(), task.getTaskType(), task.getName());
            return formatAddJarSQL(hdfsPath, task.getMainClass());
        }
        List<Long> resourceIds = developTaskResourceService.getResourceIdList(task.getId(), ResourceType.PYTHON.getType());
    ...
    public List<EScheduleJobType> support() {
        return Lists.newArrayList(EScheduleJobType.SPARK_PYTHON);
    }
