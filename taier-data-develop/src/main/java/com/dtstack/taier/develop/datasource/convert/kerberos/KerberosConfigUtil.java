 ...
 *
 * @author ：wangchuan
 * date：Created in 上午10:47 2021/7/6
...
public class KerberosConfigUtil {
    ...
    /**
     * ssl keystore 配置存放文件夹命名
     * 数据源中心dataJson未传名字，下载到同一个目录无法区分 keystore 和 truststore
     *
     * @param dtCenterSourceId 数据源中心id
     * @return sftp 数据源命名
     */
    public static String getLocalSslKeyStoreDir(Long dtCenterSourceId) {
        return localSslDir + SEPARATE + "keyStore_" + Optional.ofNullable(dtCenterSourceId).orElse(0L);
    }

    /**
     * ssl truststore 配置存放文件夹命名
     * 数据源中心dataJson未传名字，下载到同一个目录无法区分 keystore 和 truststore
     *
     * @param dtCenterSourceId 数据源中心id
     * @return sftp 数据源命名
     */
    public static String getLocalSslTrustDir(Long dtCenterSourceId) {
        return localSslDir + SEPARATE + "trustStore_" + Optional.ofNullable(dtCenterSourceId).orElse(0L);
    }
