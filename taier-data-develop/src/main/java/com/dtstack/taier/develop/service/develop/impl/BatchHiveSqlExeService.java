...
public class BatchHiveSqlExeService extends BatchSparkHiveSqlExeService implements ISqlExeService {
    ...
    public ExecuteSqlParseVO batchExecuteSql(ExecuteContent executeContent) {
        ...
        for (ParseResult parseResult : parseResultList) {
            ...
            if (Objects.nonNull(parseResult.getStandardSql()) && isSimpleQuery(parseResult.getStandardSql()) && !useSelfFunction) {
                result = simpleQuery(tenantId, parseResult, currDb, userId, EScheduleJobType.HIVE_SQL);
            ...
            if (SqlType.CREATE_AS.equals(parseResult.getSqlType())) {
                ...
                if (!executeContent.isExecuteSqlLater()) {
                    ...
                    if (SqlType.CREATE.equals(parseResult.getSqlType())
                        ...
                        executeCreateTableSql(parseResult, tenantId, tenantEngine.getComponentIdentity().toLowerCase(), EScheduleJobType.HIVE_SQL);
                        sqlIdList.add(sqlResultVO);
                    } else {
                        //todo DataSourceType.HIVE
                        exeSqlDirect(executeContent, tenantId, parseResult, result, tenantEngine, DataSourceType.HIVE);
    ...
    public void checkSingleSqlSyntax(Long tenantId, String sql, String db, String taskParam) {
        checkSingleSqlSyntax(tenantId, sql, db, taskParam, EScheduleJobType.HIVE_SQL);
    }
