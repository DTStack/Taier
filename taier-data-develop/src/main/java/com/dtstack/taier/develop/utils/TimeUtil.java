...
//            return TimespanVO.builder().correct(false).msg("timespan is not null...").build();
//        }
//        // 格式不匹配
//        if (!TIMESPAN_STR_REGEX_PATTERN.matcher(timespan.trim()).matches()) {
//            return TimespanVO.builder().correct(false).msg("timespan format error. examples: 15m、15d、1d1m、1d1m1s...").build();
//        }
...
//                return TimespanVO.builder().correct(false).msg(String.format("type '%s' in timespan repeat...", type)).build();
//            }
//            ETimeCarry timeCarry = ETimeCarry.getTimeCarryByType(type);
//            if (timeCarry.getFlag() > timeFlag) {
//                return TimespanVO.builder().correct(false).msg("press y->w->d->h->m->s sequential input. examples: 1d1m1s, example as '1s1m' is not true").build();
//            }
...
//        }
//        return TimespanVO.builder().correct(true).span(span * 1000).formatResult(formatTimespan.toString()).build();
//    }
