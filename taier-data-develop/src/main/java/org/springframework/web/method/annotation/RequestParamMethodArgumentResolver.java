...
public class RequestParamMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver
	...
	/**
	 * Create a new {@link org.springframework.web.method.annotation.RequestParamMethodArgumentResolver} instance.
	...
	protected Object resolveName(String name, MethodParameter parameter, NativeWebRequest request) throws Exception {
		...
		if (arg == null) {
			JSONObject requestBody = (JSONObject) servletRequest.getAttribute(DtRequestWrapperFilter.DT_REQUEST_BODY);

			if (requestBody != null) {
				arg = requestBody.get(name);
			}
		}
		...
		if (arg == null) {
			...
			if (!StringUtils.isEmpty(header)) {
				arg  = PublicUtil.paramToMap(header).get(name);
			}
		}
		return arg;
	}
