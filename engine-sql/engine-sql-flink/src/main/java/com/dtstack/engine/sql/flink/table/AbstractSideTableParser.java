...
import com.dtstack.engine.sql.flink.table.side.AbstractSideTableInfo;
import com.dtstack.engine.sql.flink.util.DtMathUtil;
...
public abstract class AbstractSideTableParser extends AbstractTableParser {
    ...
    protected void parseCacheProp(AbstractSideTableInfo sideTableInfo, Map<String, Object> props){
        if(props.containsKey(AbstractSideTableInfo.CACHE_KEY.toLowerCase())){
            String cacheType = DtMathUtil.getString(props.get(AbstractSideTableInfo.CACHE_KEY.toLowerCase()));
            if(cacheType == null){
                return;
            }
            ...
            if(props.containsKey(AbstractSideTableInfo.CACHE_SIZE_KEY.toLowerCase())){
                Integer cacheSize = DtMathUtil.getIntegerVal(props.get(AbstractSideTableInfo.CACHE_SIZE_KEY.toLowerCase()));
                if(cacheSize < 0){
            ...
            if(props.containsKey(AbstractSideTableInfo.CACHE_TTLMS_KEY.toLowerCase())){
                Long cacheTTLMS = DtMathUtil.getLongVal(props.get(AbstractSideTableInfo.CACHE_TTLMS_KEY.toLowerCase()));
                if(cacheTTLMS < 1000){
            ...
            if(props.containsKey(AbstractSideTableInfo.PARTITIONED_JOIN_KEY.toLowerCase())){
                Boolean partitionedJoinKey = DtMathUtil.getBoolean(props.get(AbstractSideTableInfo.PARTITIONED_JOIN_KEY.toLowerCase()));
            ...
            if(props.containsKey(AbstractSideTableInfo.CACHE_MODE_KEY.toLowerCase())){
                String cachemode = DtMathUtil.getString(props.get(AbstractSideTableInfo.CACHE_MODE_KEY.toLowerCase()));
            ...
            if(props.containsKey(AbstractSideTableInfo.ASYNC_CAP_KEY.toLowerCase())){
                Integer asyncCap = DtMathUtil.getIntegerVal(props.get(AbstractSideTableInfo.ASYNC_CAP_KEY.toLowerCase()));
                if(asyncCap < 0){
            ...
            if(props.containsKey(AbstractSideTableInfo.ASYNC_TIMEOUT_KEY.toLowerCase())){
                Integer asyncTimeout = DtMathUtil.getIntegerVal(props.get(AbstractSideTableInfo.ASYNC_TIMEOUT_KEY.toLowerCase()));
                if (asyncTimeout<0){
            ...
            if(props.containsKey(AbstractSideTableInfo.ASYNC_FAIL_MAX_NUM_KEY.toLowerCase())){
                Long asyncFailMaxNum = DtMathUtil.getLongVal(props.get(AbstractSideTableInfo.ASYNC_FAIL_MAX_NUM_KEY.toLowerCase()));
            ...
            if(props.containsKey(AbstractSideTableInfo.CONNECT_RETRY_MAX_NUM_KEY.toLowerCase())){
                Integer connectRetryMaxNum = DtMathUtil.getIntegerVal(props.get(AbstractSideTableInfo.CONNECT_RETRY_MAX_NUM_KEY.toLowerCase()));
