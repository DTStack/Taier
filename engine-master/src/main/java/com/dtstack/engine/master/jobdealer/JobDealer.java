...
public class JobDealer implements InitializingBean, ApplicationContextAware {
    ...
    public void saveCache(JobClient jobClient, String jobResource, int stage, boolean insert) {
        ...
        if (insert) {
            scheduleJobCacheService.insert(jobClient.getJobId(), jobClient.getEngineType(), jobClient.getComputeType().getType(), stage, jobClient.getParamAction().toString(), nodeAddress, jobClient.getJobName(), jobClient.getPriority(), jobResource, jobClient.getTenantId());
