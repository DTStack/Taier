...
public class ClusterServiceTest extends AbstractTest {
    ...
    public void testGetCluster() throws Exception{
        ...
        //测试组件联通性
        ComponentTestResult componentTestResult = componentService.testConnect(yarn.getComponentTypeCode(), yarnString, testClusterName, yarn.getHadoopVersion(), engineId, null, sftpMap,null,Collections.emptyMap(),null);
        ...
        //loadTemplate
        String typeName = componentService.convertComponentTypeToClient(testClusterName, EComponentType.SPARK.getTypeCode(),"210",null,Collections.emptyMap(),null);
    ...
    public void testPluginJson(){
        ...
        JSONObject dumy = clusterService.pluginInfoJSON(null, "dummy", 1L, 0,Collections.emptyMap());
    ...
    private void checkQueryWithUicTenantId(Tenant tenant) {
        ...
        ClusterVO clusterVO = clusterService.clusterExtInfo(dtUicTenantId,false);
        Assert.assertNotNull(clusterVO);
        JSONObject infoJSON = clusterService.pluginInfoJSON(dtUicTenantId, "hadoop", null, null,Collections.emptyMap());
       ...
       //查询集群信息
        ...
        Assert.assertNotNull(engineTenants.getData());
        //查询集群组件信息
        JSONArray componentsJson = JSONObject.parseArray(JSON.toJSONString(componentService.listConfigOfComponents(tenant.getDtUicTenantId(), MultiEngineType.HADOOP.getType(),null)));
        Assert.assertNotNull(componentsJson);
        ...
        JSONObject sparkConf = clusterService.pluginInfoJSON(tenant.getDtUicTenantId(), EngineTypeComponentType.SPARK.name(), null, null,Collections.emptyMap());
        ...
        Assert.assertNotNull(clusterService.hiveInfo(dtUicTenantId, true,Collections.emptyMap()));
    ...
    private ClusterVO testGetCluster(ClusterVO clusterVO) {
        //测试yarn 和hdfs是否存在
        //单个
        ClusterVO cluster = clusterService.getCluster(clusterVO.getClusterId(), true,false);
        ...
        List<IComponentVO> components = commonSchedule.get().getComponents();
        Assert.assertNotNull(components);
        Optional<IComponentVO> sftpComponent = components.stream().filter(c -> c.getComponentTypeCode().equals(EComponentType.SFTP.getTypeCode())).findAny();
        ...
        Optional<IComponentVO> yarnComponent = resourceSchedule.get().getComponents().stream().filter(c -> c.getComponentTypeCode().equals(EComponentType.YARN.getTypeCode())).findAny();
    ...
    private ComponentVO testAddYarn(ClusterVO clusterVO) {
        ...
        componentService.addOrUpdateComponent(clusterVO.getClusterId(), "{\"path\":\"/data/sftp\",\"password\":\"123\",\"auth\":\"1\",\"port\":\"22\",\"host\":\"172.16.100.168\",\"username\":\"root\"}",
                null, "hadoop2", "", sftpTemplate, EComponentType.SFTP.getTypeCode(),null,"","",true,true,null);
        return componentService.addOrUpdateComponent(clusterVO.getClusterId(), "{\"yarn.resourcemanager.zk-address\":\"172.16.100.216:2181,172.16.101.136:2181,172.16.101.227:2181\",\"yarn.resourcemanager.admin.address.rm1\":\"172.16.100.216:8033\",\"yarn.resourcemanager.webapp.address.rm2\":\"172.16.101.136:8088\",\"yarn.log.server.url\"" +
                ...
                , null, "hadoop2", "", "[]", EComponentType.YARN.getTypeCode(),null,"","",true,true,null);
    }
    ...
    private ComponentVO testAddHdfs(ClusterVO clusterVO) {
        return componentService.addOrUpdateComponent(clusterVO.getClusterId(), "{\"fs.defaultFS\":\"hdfs://ns1\",\"dfs.replication\":\"1\",\"dfs.ha.fencing.methods\":\"sshfence\",\"dfs.client.failover.proxy.provider.ns1\":\"org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider\",\"dfs.ha.fencing.ssh.private-key-files\":\"~/.ssh/id_rsa\",\"dfs.nameservices\":\"ns1\",\"fs.hdfs.impl.disable.cache\":\"true\",\"dfs.safemode.threshold.pct\":\"0.5\",\"dfs.ha.namenodes.ns1\":\"nn1,nn2\",\"dfs.namenode.name.dir\":\"file:/data/hadoop/hdfs/name\",\"dfs.journalnode.rpc-address\":\"0.0.0.0:8485\",\"fs.trash.interval\":\"14400\",\"dfs.journalnode.http-address\":\"0.0.0.0:8480\",\"dfs.namenode.rpc-address.ns1.nn2\":\"172.16.101.136:9000\",\"dfs.namenode.rpc-address.ns1.nn1\":\"172.16.100.216:9000\"," +
                ...
                , null, "hadoop2", "", "[]", EComponentType.HDFS.getTypeCode(),null,"","",true,true,null);
    }
    ...
    private void testAddHiveWithKerberos(ClusterVO clusterVO) {
        componentService.addOrUpdateComponent(clusterVO.getClusterId(),"{\"jdbcUrl\":\"jdbc:hive2://eng-cdh3:10001/default;principal=hive/eng-cdh3@DTSTACK.COM\",\"maxJobPoolSize\":\"\",\"minJobPoolSize\":\"\",\"password\":\"\",\"queue\":\"\",\"username\":\"\"}",
                ...
                9,EComponentType.HDFS.getTypeCode(),"hive/eng-cdh3@DTSTACK.COM","hive/eng-cdh3@DTSTACK.COM",true,true,null);
    }
    ...
    private ComponentVO testAddSpark(ClusterVO clusterVO) {
        ...
        return componentService.addOrUpdateComponent(clusterVO.getClusterId(), "", null, "hadoop2", "", componentTemplate, EComponentType.SPARK.getTypeCode(),null,"","",true,true,null);
    }
