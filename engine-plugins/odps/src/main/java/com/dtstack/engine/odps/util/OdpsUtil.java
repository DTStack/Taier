...
public class OdpsUtil {
    public static Odps initOdps(Map<String,String> odpsConfig) {
        ...
        if(StringUtils.isBlank(accessId)) {
            throw new RdosDefineException("accessId is required");
        }

        String accessKey = odpsConfig.get(ConfigConstant.ACCESS_KEY);
        if(StringUtils.isBlank(accessKey)) {
            throw new RdosDefineException("accessKey is required");
        }

        String project = odpsConfig.get(ConfigConstant.PROJECT);
        if(StringUtils.isBlank(project)) {
            throw new RdosDefineException("project is required");
        }
        ...
        if (accountType.equalsIgnoreCase(ConfigConstant.DEFAULT_ACCOUNT_TYPE)) {
            ...
            throw new RdosDefineException(String.format("unsupported account type:[%s]. Current only support aliyun, taobao.", accountType));
        }
