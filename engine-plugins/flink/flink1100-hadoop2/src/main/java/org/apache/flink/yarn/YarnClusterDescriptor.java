...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	private ApplicationReport startAppMaster(
		...
		if (!this.providedUserJarFiles.isEmpty()) {
			userJarFiles.addAll(providedUserJarFiles);
		}
		...
		final List<String> userClassPaths = uploadAndRegisterFiles(
			...
			userJarInclusion == YarnConfigOptions.UserJarInclusion.DISABLED ?
					DEFAULT_FLINK_USR_LIB_DIR : Path.CUR_DIR,
			envShipFileList);
		...
		File tmpFileDir =  new File(System.getProperty("user.dir") + File.separator + "tmp1100");
