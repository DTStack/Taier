...
class YarnApplicationFileUploader implements AutoCloseable {
    ...
    static YarnApplicationFileUploader from(
            ...
            throws IOException {
        return new YarnApplicationFileUploader(
                fileSystem, homeDirectory, providedLibDirs, applicationId, fileReplication, flinkConfiguration);
    }
    ...
    private Path copyToRemoteApplicationDir(
        ...
        if (remoteFlinkJarPath != null && remoteFlinkJarPath.startsWith(ConfigConstant.PREFIX_HDFS)) {
            ...
            if (path.startsWith(flinkJarPath)) {
                path = StringUtils.replace(path, flinkJarPath, remoteFlinkJarPath);
                return new Path(path);
            } else if (path.startsWith(flinkPluginRoot)) {
                path = StringUtils.replace(path, flinkPluginRoot, remotePluginRootDir);
                return new Path(path);
            }
        }
