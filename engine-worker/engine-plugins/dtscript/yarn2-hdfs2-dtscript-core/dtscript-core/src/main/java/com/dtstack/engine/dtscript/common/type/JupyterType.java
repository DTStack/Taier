...
public class JupyterType extends AbstractAppType {
    ...
    /** log file directory */
    private final static String JUPYTER_LOG_DIR_SUFFIX = "/log";
    ...
    /** debug configuration*/
    private final static String JUPYTER_LOG_DEBUG = "jupyter.log.debug";
    /** log file name */
    private final static String JUPYTER_LOG_FILENAME = "jupyter.log";
    ...
    public String buildCmd(ClientArguments clientArguments, YarnConfiguration conf) {
        ...
        boolean allowDebug = conf.getBoolean(JUPYTER_LOG_DEBUG, false);
        ...
        String jupyterLogDir = jupyterProject + JUPYTER_LOG_DIR_SUFFIX;
        ...
        bashScript.append("  mkdir -p \"").append(jupyterWorkspace).append("\"").append(LINEFEED);
        bashScript.append("fi").append(LINEFEED);
        bashScript.append("if [ ! -d \"").append(jupyterConfDir).append("\" ]; then").append(LINEFEED);
        bashScript.append("  mkdir -p \"").append(jupyterConfDir).append("\"").append(LINEFEED);
        bashScript.append("fi").append(LINEFEED);
        bashScript.append("if [ ! -d \"").append(jupyterLogDir).append("\" ]; then").append(LINEFEED);
        bashScript.append("  mkdir -p \"").append(jupyterLogDir).append("\"").append(LINEFEED);
        bashScript.append("fi").append(LINEFEED);
        ...
        if (allowDebug) {
            bashScript.append(" --debug");
        }
        bashScript.append(" > ").append(jupyterLogDir).append("/").append(JUPYTER_LOG_FILENAME).append(" 2>&1 ");
