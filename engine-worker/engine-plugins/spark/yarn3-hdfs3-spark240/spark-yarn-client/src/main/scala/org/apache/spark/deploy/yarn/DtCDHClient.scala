...
private[spark] class DtCDHClient(
  ...
  private val distCacheMgr = new DtClientDistributedCacheManager()
  ...
  def createApplicationSubmissionContext(
    ...
    val amResources =
      if (isClusterMode) {
        sparkConf.getAllWithPrefix(dtconfig.YARN_DRIVER_RESOURCE_TYPES_PREFIX).toMap
      } else {
        sparkConf.getAllWithPrefix(dtconfig.YARN_AM_RESOURCE_TYPES_PREFIX).toMap
      }
  ...
  def prepareLocalResources(
    ...
    if (sparkArchive.isDefined) {
      ...
      //TODO require(!Utils.isLocalUri(archive), s"${SPARK_ARCHIVE.key} cannot be a local URI.")
  ...
  def loadConfFromLocal(hadoopConfFiles: HashMap[String, File]): Unit ={
  ...
  def loadHadoopConf(hadoopConfFiles: HashMap[String, File]): Unit ={
  ...
  private def createConfArchive(): File = {
    ...
    loadHadoopConf(hadoopConfFiles)
...
private object XXUtils extends Logging{
  def isLocalUri(uri: String): Boolean = {
    val LOCAL_SCHEME = "local"
    uri.startsWith(s"$LOCAL_SCHEME:")
  }
...
private object DtCDHClient extends Logging {
  ...
  private[yarn] def populateClasspath(
    ...
    if (sparkConf.get(SPARK_ARCHIVE).isEmpty) {
      sparkConf.get(SPARK_JARS).foreach { jars =>
        jars.filter(XXUtils.isLocalUri).foreach { jar =>
          val uri = new URI(jar)
