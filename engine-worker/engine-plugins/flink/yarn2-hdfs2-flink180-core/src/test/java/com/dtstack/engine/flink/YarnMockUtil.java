...
public class YarnMockUtil {
    ...
    public static ClusterClient mockClusterClient() throws Exception {
        ...
        JobSubmissionResult result = PowerMockito.mock(JobSubmissionResult.class);
        when(result.isJobExecutionResult()).thenReturn(false);
        when(result.getJobID()).thenReturn(new JobID());

        ClusterClient clusterClient = PowerMockito.mock(ClusterClient.class);
        when(clusterClient.run(any(PackagedProgram.class), any(int.class))).thenReturn(result);
        when(clusterClient.getClusterId()).thenReturn(PowerMockito.mock(ApplicationIdPBImpl.class));
        when(clusterClient.getWebInterfaceURL()).thenReturn("http://dtstack01:8088/proxy/");
        when(clusterClient.cancelWithSavepoint(any(JobID.class), any(String.class))).thenReturn("save path");
    ...
    public static YarnClusterDescriptor mockYarnClusterDescriptor(ClusterClient clusterClient) throws Exception {
        ...
        when(yarnClusterDescriptor.retrieve(any(ApplicationId.class))).thenReturn(clusterClient);
