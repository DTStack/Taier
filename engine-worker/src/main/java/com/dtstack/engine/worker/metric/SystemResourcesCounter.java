...
public class SystemResourcesCounter extends Thread {
	...
	public SystemResourcesCounter(long duration) {
		probeIntervalMs = TimeUnit.MILLISECONDS.toMillis(duration);
		Preconditions.checkArgument(this.probeIntervalMs > 0);
	...
	private void calculateCPUUsage(CentralProcessor processor) {
		...
		Preconditions.checkArgument(load.length == cpuUsagePerProcessor.length());
	...
	private void calculateNetworkUsage(NetworkIF[] networkIFs) {
		Preconditions.checkArgument(networkIFs.length == receiveRatePerInterface.length());
