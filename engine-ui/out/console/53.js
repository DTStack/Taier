(window["webpackJsonp_console-ui"] = window["webpackJsonp_console-ui"] || []).push([[53],{
...
function fromMarkdownString(entry) {
    return {
        kind: (typeof entry === 'string' ? MarkupKind.PlainText : MarkupKind.Markdown),
...
function fromCompletionItem(entry) {
    ...
    if (entry.range) {
        item.textEdit = TextEdit.replace(fromRange(entry.range), item.insertText);
    }
    return item;
}
