...
class ResForm extends React.Component<any, any> {
	...
	renderFormItem = () => {
		...
		const { defaultData, isEditExist, isCreateFromMenu, isCreateNormal, isCoverUpload } =
			this.props;
		let accept: any;
		if (fileType != RESOURCE_TYPE.OTHER) {
		...
		if (!isCoverUpload) {
			return [
				<FormItem {...formItemLayout} label="资源名称" key="resourceName">
				...
				<FormItem {...formItemLayout} label="资源类型" key="resourceType">
				...
				<FormItem {...formItemLayout} label="选择存储位置" key="nodePid">
			...
			return [
				...
				<FormItem {...formItemLayout} label="文件名" key="originFileName">
...
class ResModal extends React.Component<any, any> {
	...
	render() {
		const { isModalShow, resourceTreeData, defaultData, isCoverUpload } = this.props;
