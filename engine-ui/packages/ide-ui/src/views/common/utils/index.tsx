...
import { FileTypes, IStatusBarItem, TreeNodeModel, Float } from '@dtinsight/molecule/esm/model';
...
export function updateStatusBarLanguage(item: IStatusBarItem) {
	...
	const languageStatus = states.rightItems.find((item) => item.id === 'language');
	if (languageStatus) {
 ...
 * @returns
 */
export async function getCatalogueViaNode(node: Pick<CatalogueDataProps, 'id' | 'catalogueType'>) {
	return new Promise<any>(async (resolve, reject) => {
		if (!node) reject(new Error('[getCatalogueViaNode]: failed to get catelogue'));
 ...
 * @returns
 */
export function getRootFolderViaSource(data: CatalogueDataProps[], source: Source) {
	switch (source) {
		case 'task': {
			return data
				.find((item) => item.catalogueType === 'TaskManager')
				?.children?.find((item) => item.catalogueType === 'TaskDevelop');
		}
	...
	switch (source) {
		...
		case 'resource': {
			...
			return new TreeNodeModel({
				...
				fileType,
				icon: fileIcon(
					source === 'task' ? catalogue.taskType : catalogue.resourceType,
					source,
				),
