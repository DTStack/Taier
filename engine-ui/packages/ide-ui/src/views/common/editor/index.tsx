		...
		switch (menuId) {
			case TASK_RUN_ID: {
				...
				if (value) {
					...
					const { tabs, currentTab: currentTaskId } = (store.getState() as any)
						.workbenchReducer;
						...
						execSql(
							...
							.then(() => {
								const results: {
									...
									(store.getState() as any).editor.console[task.id]?.results ||
									[];
								...
								results.forEach((res) => {
									const panel = molecule.panel.getPanel(res.jobId);
									if (!panel) {
