...
class DataSync extends React.Component<any, any> {
    ...
    onVariablesChange = (oldProps: any, nextProps: any) => {
        ...
        if (
            (Number(oldSource.sourceId) !== Number(sourceMap.sourceId) || Number(oldTarget.sourceId) !== Number(targetMap.sourceId)) &&
            oldSource.sourceId && oldTarget.sourceId
        ) {
            return false
        }
        ...
        const isObjectsChange = !!newSourceObejct && !!oldSourceObject && !isEqual(newSourceObejct, oldSourceObject)
        const isObjectChange = !!oldTargetObject && !!newTargetObject && oldTargetObject !== newTargetObject
        const isStartChange = !!oldSourceStart && !!newSourceStart && newSourceStart !== oldSourceStart
    ...
    getJobData = (params: any) => {
        ...
        //         } else {
        //             this.props.getDataSyncSaved(dataSyncSaved);
        //             if (JSON.stringify(dataSyncSaved.targetMap) === '{}' || !dataSyncSaved.targetMap) {
        //                 this.navtoStep(0)
        //             }else {
        //                 this.navtoStep(dataSyncSaved.currentStep.step)
        //             }
