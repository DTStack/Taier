...
import { formItemLayout, DATA_SOURCE, hdfsFieldTypes, hbaseFieldTypes } from '../../../comm/const';
...
class KeyForm extends React.Component<any, any> {
	...
	renderFormItems = () => {
		const { keyModal, dataType, sourceColumnFamily, targetColumnFamily } = this.props;
		...
		if (isReader) {
					...
					return [
						...
						<FormItem {...formItemLayout} label="类型" key="type">
							{getFieldDecorator('type', {
								...
								initialValue: (editField && editField.type) || 'STRING',
							})(<Select placeholder="请选择类型">{renderHDFSOptions()}</Select>)}
						</FormItem>,
					];
				}
			...
			switch (dataType) {
				...
				case DATA_SOURCE.S3: {
					return [
						<FormItem {...formItemLayout} label="字段名" key="keyName">
							{getFieldDecorator('key', {
								rules: [
									{
										required: true,
...
class KeyMapModal extends React.Component<any, any> {
	...
	render() {
		const { title, visible, keyModal, dataType, sourceColumnFamily, targetColumnFamily } =
			this.props;
		return (
			<Modal title={title} visible={visible} onOk={this.submit} onCancel={this.cancel}>
				<KeyFormWrapper
