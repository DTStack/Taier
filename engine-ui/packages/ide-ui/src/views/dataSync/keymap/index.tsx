...
import { Button, Row, Col, Input, Tooltip, message, Icon, Modal } from 'antd';
import { select, selectAll, mouse } from 'd3-selection';
import { Resize, ScrollText } from 'dt-react-component';
import { debounceEventHander, isHdfsType } from '../../../comm';

import { keyMapActions } from '../../../controller/dataSync/offlineAction';
import { Utils } from '@dtinsight/dt-utils';
import Api from '../../../api';

import { hdfsFieldTypes, hbaseFieldTypes, DATA_SOURCE } from '../../../comm/const';
...
class Keymap extends React.Component<any, any> {
	...
	resetActiveLine() {
		this.$activeLine.attr('x1', -10).attr('y1', -10).attr('x2', -10).attr('y2', -10);
	}
	...
	loadColumnFamily = () => {
		const { sourceMap, targetMap, sourceSrcType, targetSrcType } = this.props;
		const ctx = this;
	...
	debounceRowkeyChange = debounceEventHander(this.hbaseRowKeyChange.bind(this), 50, {
		maxWait: 2000,
	});
	...
	renderSource() {
		...
		const { sourceCol, sourceSrcType, sourceFileType, removeSourceKeyRow, readonly } =
			this.props;
		...
		const renderTableRow = (sourceType?: any, col?: any, i?: any) => {
			const removeOption = (
				<div className="remove-cell" onClick={() => removeSourceKeyRow(col, i)}>
					<Tooltip title="删除当前列">
						<Icon type="minus" />
					</Tooltip>
				</div>
					...
					return (
						<div>
							...
							<div className="cell" title={typeValue}>
								...
								{col && col.isPart && (
									<img
										src="public/img/primary-key.svg"
										style={{
											float: 'right',
											marginTop: '-26px',
										}}
					...
					case DATA_SOURCE.HBASE:
						footerContent = (
							<span>
								<span
									className="col-plugin"
									onClick={this.initAddKeyRow.bind(this, true, sourceFileType)}
								>
									+添加字段
								</span>
								&nbsp;
	...
	renderTarget() {
		...
		const renderTableRow = (targetType?: any, col?: any, i?: any) => {
			...
			switch (targetType) {
				case DATA_SOURCE.HDFS:
				case DATA_SOURCE.S3: {
					const name: any = col ? <ScrollText value={col.key} /> : '字段名称';
				...
				case DATA_SOURCE.FTP: {
					const column: any = col ? <ScrollText value={col.key} /> : '字段名称';
					const type = col ? col.type.toUpperCase() : '类型';
					return (
						<div>
							<div className="cell" title={column}>
					...
					case DATA_SOURCE.HBASE:
						footerContent = (
							<div>
								<span
									className="col-plugin"
									onClick={this.initAddKeyRow.bind(this, false, targetFileType)}
								>
									+添加字段
								</span>
								&nbsp;
			...
			content: (
				<div>
					<img
						...
						src="public/img/keymap_help.jpg"
					/>
				</div>
			),
	...
	renderKeyModal = () => {
		const { keyModal, keyModalVisible, sourceColumnFamily, targetColumnFamily } = this.state;
		...
		if (operation === 'add') {
			if (dataType === DATA_SOURCE.HBASE) {
				...
				title = '添加FTP字段';
			} else if (dataType === DATA_SOURCE.S3) {
				title = '添加AWS S3字段';
			}
		...
		if (isNotHBase) {
			...
			typeCol &&
				typeCol.forEach((item: any) => {
					...
					if (field !== undefined)
						initialVal += `${item.cf || '-'}:${field}:${item.type},\n`;
				});
		}
		return initialVal;
	};
	...
	renderBatchModal = () => {
		const { batchModal, batchSourceModal, sourceColumnFamily, targetColumnFamily } = this.state;
		const { sourceSrcType, targetSrcType, sourceCol, targetCol } = this.props;
			...
			<Resize onResize={this.resize}>
				<div style={{ margin: '0 20px' }}>
					...
					>
						...
						&nbsp;
						{noKeyMapping ? (
							<a onClick={this.keymapHelpModal}>如何快速配置字段映射？</a>
						) : (
							''
						)}
					</p>
						...
						<Col span={3}>
							{!this.props.readonly ? (
								<div className="m-buttons">
									<Button type={rowBtnType} onClick={this.setRowMap.bind(this)}>
				...
				for (let i = 0; i < arr.length; i++) {
					...
					if (val && val.length === 2) {
						const res = source.find(
							(item: any) => item.cf === val[0] && item.key === val[1],
						);
						if (!res) {
