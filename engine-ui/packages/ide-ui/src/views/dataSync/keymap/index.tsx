...
import {
    debounceEventHander, isHdfsType
} from '../../../comm'
...
class Keymap extends React.Component<any, any> {
    ...
    bindEvents () {
        ...
        $dagL.on('mousedown', (d: any, i: any, nodes: any) => {
            const sx = padding; const sy = (i + 1.5) * h
            $line.attr('x1', sx)
                .attr('y1', sy)
                .attr('x2', sx)
                .attr('y2', sy)
    ...
    renderSource () {
        ...
        const renderTableRow = (sourceType?: any, col?: any, i?: any) => {
            ...
            const typeValue = col ? col.value ? `常量(${col.type})` : `${col.type ? col.type.toUpperCase() : ''}${col.format ? `(${col.format})` : ''}` : ''
            const type = col ? <ScrollText value={typeValue} /> : '类型'

            switch (sourceType) {
                case DATA_SOURCE.HDFS:
                case DATA_SOURCE.S3: {
                    const name: any = col ? <ScrollText value={col.index !== undefined ? col.index : col.value ? `'${col.key}'` : col.key} /> : '索引位'
                    return <div>
                ...
                case DATA_SOURCE.HBASE: {
                    const name: any = col ? <ScrollText value={col.value ? `'${col.key}'` : col.key} /> : '列名/行健'
                    const cf = col ? col.cf : '列族'

                    // 仅允许常量删除操作
                    const opt = col && col.key === 'rowkey'
                ...
                case DATA_SOURCE.HIVE_3: {
                    const name: any = col ? <ScrollText value={col.value ? `'${col.key}'` : col.key} /> : '字段名称'
                    // 仅允许常量删除操作
                    const opt = col && col.value
                        ? cellOperation(removeOption, editOption)
                        : cellOperation(null, editOption)
                    return <div>
                        <div className="cell" >{name}</div>
                ...
                case DATA_SOURCE.FTP: {
                    const name: any = col ? <ScrollText value={col.index !== undefined ? col.index : col.value ? `'${col.key}'` : col.key} /> : '字段序号'
                    return <div>
                        <div className="cell" >{name}</div>
                        <div className="cell" title={typeValue}>{type}</div>
                        <div className="cell">
    ...
    next (cb: any) {
        const { keymap, targetSrcType, targetCol, isNativeHive, saveDataSyncToTab, dataSync, currentTabData } = this.props
        const { source, target } = keymap
