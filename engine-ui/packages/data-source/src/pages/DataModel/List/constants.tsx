	...
	return (title, action) => {
		return (
			<a
				...
				onClick={() => !isPublished && typeof action === 'function' && action()}
			>
				{title}
			</a>
		);
	...
	return [
		{
			...
			render: (modelName, record) => {
				return (
					<a onClick={() => handleModelNameClick(record.id)} className="btn-link">
						{modelName}
					</a>
				);
			},
		...
		},
		{
			...
			render: (text, record) => {
				...
				const btnDelete = wrapper('删除', () => handleDeleteBtnClick(record.id));
