...
const RelationTableModal = (props: IPropsRelationTableModal) => {
	const { form, updateTypeList, tableList, value, modelDetail, mode, cref } = props;
	const { getFieldsValue, getFieldDecorator, setFieldsValue, validateFields } = form;
	const [schemaList, setSchemaList] = useState<string[]>([]);
	const [relationTableList, setRelationTableList] = useState<{ tableName: string }[]>([]);
	...
	useEffect(() => {
		...
		const leftTable =
			value.leftSchema && value.leftTable ? tableParser.encode(mainTable) : undefined;
		setFieldsValue({
			leftTable,
	...
	// 请求接口判断表是否为分区表
	const isPartition = async (dsId: number, schema: string, tableName: string) => {
	...
	useEffect(() => {
		isPartition(modelDetail.dsId, currentFormValue.schema, currentFormValue.table);
