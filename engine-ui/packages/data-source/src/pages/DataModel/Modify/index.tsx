...
const stepRender = (current: EnumModifyStep, params: any) => {
	const { childRef, modelDetail, globalStep, mode, setModelDetail, setDisabled } = params;
	switch (current) {
		...
		case EnumModifyStep.DIMENSION_STEP:
			return <FieldSelect cref={childRef} step={current} modelDetail={modelDetail} />;
		case EnumModifyStep.METRIC_STEP:
			return <FieldSelect cref={childRef} step={current} modelDetail={modelDetail} />;
		case EnumModifyStep.SETTING_STEP:
			return <PartitionField cref={childRef} modelDetail={modelDetail} mode={mode} />;
	}
};
...
const Modify = (props: IPropsModify) => {
	...
	const saveDataModel = async (params?: Partial<IModelDetail>, callback?: Function) => {
		try {
