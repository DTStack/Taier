...
const PartitionField = (props: IPropsPartitionField) => {
	...
	const isEnabledPartition =
		modelDetail.modelStatus === EnumModelStatus.UNRELEASE || mode === EnumModifyMode.ADD;
	// TODO:树形列表功能

	const modelPartitionParser = (data): void => {
		const dateColString = data.modelPartition?.datePartitionColumn?.columnName || '';
		const datePartition = columnSrtingParser.decode(dateColString);
		data.modelPartition.datePartitionColumn = datePartition;
		const timeColString = data.modelPartition?.timePartitionColumn?.columnName || '';
	...
	useEffect(() => {
		...
		setFieldsValue({
			modelPartition: {
				...
				datePartitionColumn: {
					columnName: dateColumn ? columnSrtingParser.encode(dateColumn) : undefined,
				},
	...
	const rules = useCallback(
		(msg: string) => [{ required: currentForm.modelPartition?.timePartition, message: msg }],
	...
	useEffect(() => {
		if (currentForm.modelPartition?.timePartition === true) {
			...
			const modelPartition = {
				...
				timePartitionColumn: {
					columnName:
						timeColumn === null ? undefined : columnSrtingParser.encode(timeColumn),
				},
			};
			setFieldsValue({
				modelPartition,
		...
		<Form className=" padding-top-20 partition-field dm-form" {...layout}>
			<Row className="mb-12">
				<Col span={5}>
					...
					<Tooltip placement="right" title="如果数据模型不需要增量更新，可不设置分区">
