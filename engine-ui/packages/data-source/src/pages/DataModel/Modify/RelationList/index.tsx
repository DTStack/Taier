...
import React, { useState, useMemo, useImperativeHandle, useRef, useEffect } from 'react';
...
const RelationList = (props: IPropsRelationList) => {
	...
	const onRelationListDelete = (id: number | string) => {
		Modal.confirm({
			title: <span className="cus-modal margin-left-40">确认删除该条关联记录吗？</span>,
			content: (
			...
			icon: <i className="cus-modal icon iconfont2 iconFilltianchong_Close-Circle-Fill" />,
		});
	};
...
export default React.memo(React.forwardRef(RelationList), (curProps, nextProps) => {
