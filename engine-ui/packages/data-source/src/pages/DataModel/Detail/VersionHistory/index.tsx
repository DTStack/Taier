...
const VersionHistory = (props: IPropsVersionHistory) => {
	...
	const [versionHistoryList, setVersionHistoryList] = useState<IVersionHistoryItem[]>([]);
	...
	const getVersionHistoryList = async (params) => {
		setLoading(true);
		try {
			const { success, data, message } = await API.getVersionHistoryList(params);
			if (success) {
	...
	const hadnleModalCompareAction = ({ type, payload }: IModalAction<[string, string]>) => {
		switch (type) {
			case 'CLOSE':
	...
	return (
		<div className="version-history">
			...
			<DtModal
				...
				onCancel={() => handleModalDetailAction({ type: EnumModalActionType.CLOSE })}
				footer={
					<Button
						type="primary"
						onClick={() => handleModalDetailAction({ type: EnumModalActionType.CLOSE })}
					>
						确定
					</Button>
				}
			...
			>
				<div className="margin-top-24">
					<VersionCompare modelId={modelId} versions={visibleModelCompare.versions} />
				</div>
			</DtModal>
		</div>
	);
