...
export const relationViewTreeParser = (modelDetail: Partial<IModelDetail>): IRelationTree => {
	const { columns, joinList } = modelDetail;
	const getColumnsByTableAlias = getColumnsByTableAliasGenerator(columns, joinList);
	const createTableNodeChildren = (parentNodeAlias: string, joinInfoList: any[]) => {
		const list = joinList.filter((item) => item.leftTableAlias === parentNodeAlias);
...
const RelationView = (props: IPropsRelationView) => {
	...
	const tableRender = (graph) => {
		return (
			...
			const tableNameCellColor = _tableType === EnumTableType.PRIMARY ? '#3F87FF' : '#FFB310';
			...
			if (_tableType === EnumTableType.PRIMARY) refCenterCell.current = parentCell;
	...
	const getLabel = (cell) => {
		if (cell.edge === true) {
			...
			const offsetY =
				...
				(targetParent.geometry.height * targetCell.geometry.y + targetParent.geometry.y);
			const offsetX =
				sourceParent.geometry.width + sourceParent.geometry.x - targetParent.geometry.x;
			...
			loop(tree, (item) => {
				...
				if (joinInfo) {
					...
					joinPairs.forEach((joinItem) => {
						...
						const rightTableColumnList = map.get(joinItem.rightValue.tableAlias);
