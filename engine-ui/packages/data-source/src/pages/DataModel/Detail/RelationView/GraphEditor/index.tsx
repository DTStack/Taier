...
export default class GraphEditor extends React.Component<Props, State> {
	...
	tools: any[] = [
		{
			title: '居中',
			action: () => this.handleToolAction({ type: EnumToolActionType.ALIGN_CENTER }),
		...
		{
			title: '缩小',
			action: () => this.handleToolAction({ type: EnumToolActionType.ZOOM_OUT }),
			icon: 'iconOutlinedxianxing_zoom-out',
		},
		{
			title: '下载',
			action: () => this.handleToolAction({ type: EnumToolActionType.DOWNLOAD }),
	...
	handleToolAction = (toolAction: IToolAction) => {
		switch (toolAction.type) {
			case EnumToolActionType.ALIGN_CENTER:
				if (typeof this.props.alignCenter === 'function') return this.props.alignCenter();
				this.alignCenter();
				break;
	...
	downLoadImage = () => {
		...
		this.html2canvas(this.Container, name, imageAttarbute.width, imageAttarbute.height * 1.2);
	};
