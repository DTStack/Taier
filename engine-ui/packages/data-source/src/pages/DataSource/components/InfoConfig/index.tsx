...
const InfoConfig = (props) => {
		...
		validateFields(async (err, fieldsValue) => {
			if (!err) {
				...
				if (getFieldValue('kerberosFile')) {
					...
					handelParams.appTypeListString = JSON.stringify(otherParams.appTypeList);
					try {
						handelParams.dataJsonString = Base64.encode(JSON.stringify(fieldsValue));
		...
		validateFields(async (err, fieldsValue) => {
			...
			if (!err) {
				if (getFieldValue('kerberosFile')) {
					...
					let { success, message: msg, data } = await API.addDatasource(handelParams);
	...
	const validateIsEmpty = (params) => {
		return Object.keys(params).includes('') || Object.values(params).includes('');
	};
	...
	const getPrincipalsWithConf = async (kerberosFile?: any, callBack?: Function) => {
			...
			case 'InputWithCopy':
				return (
					<Form.Item label={item.label} key={index}>
						...
						<Icon className="help-doc" type="copy" onClick={() => handleCopy(item)} />
					</Form.Item>
				);
			case 'Select':
			...
			case 'RedisReact':
				return (
					<div key={index}>
						...
						{(getFieldValue('redisType') === 1 || getFieldValue('redisType') === 2) && (
							<Form.Item label="数据库">
