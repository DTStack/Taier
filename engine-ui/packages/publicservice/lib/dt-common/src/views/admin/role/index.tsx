...
class AdminRole extends React.Component<any, RoleState> {
  ...
  loadData = () => {
    ...
    const app = active;
    const haveSelected =
      (MY_APPS.RDOS == active && selectedProject) ||
      (MY_APPS.STREAM == active && streamSelectedProject) ||
      (MY_APPS.SCIENCE == active && scienceSelectedProject) ||
      (MY_APPS.API == active && apiSelectedProject) ||
      (MY_APPS.DATA_QUALITY == active && dqSelectedProject) ||
      (MY_APPS.TAG == active && tagSelectedProject);
    ...
    return [
      {
        ...
        render(text: any, record: any) {
          return (
            <Link to={`/admin/role/edit/${record.id}?app=${active}`}>
              {text}
            </Link>
          );
        },
  ...
  renderPane = () => {
    ...
    return (
      <Card bordered={false} hoverable={false} title={title} extra={''}>
        <Table
          rowKey="id"
          className="m-table"
