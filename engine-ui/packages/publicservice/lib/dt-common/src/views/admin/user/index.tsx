...
class AdminUser extends React.Component<any, UserManaState> {
    ...
    Api.getProjects(app).then((res: any) => {
      ...
      if (res.code === 1) {
        ...
        if (app == MY_APPS.STREAM) {
          cookiesProject = getNotNullProject(
            Cookie.getCookie('stream_project_id'),
            res.data
          );
          ctx.setState(
            {
          ...
          cookiesProject = getNotNullProject(
            Cookie.getCookie('science_project_id'),
            res.data
          );
          ctx.setState(
            {
          ...
          cookiesProject = getNotNullProject(
            Cookie.getCookie('tag_project_id'),
            res.data
          );
          ctx.setState(
            {
          ...
          cookiesProject = getNotNullProject(
            Cookie.getCookie('api_project_id'),
            res.data
          );
          ctx.setState(
            {
          ...
          cookiesProject = getNotNullProject(
            Cookie.getCookie('dq_project_id'),
            res.data
          );
          ctx.setState(
            {
  ...
  loadUsersNotInProject = (userName?: any) => {
    ...
    Api.loadUsersNotInProject(active, params).then((res: any) => {
      this.setState({
        notProjectUsers: uniqBy(concat(res.data, notProjectUsers), 'userId'),
      });
    });
  };
  ...
  initColums = () => {
    ...
    const hideDel =
      active == MY_APPS.RDOS ||
      active == MY_APPS.STREAM ||
      active == MY_APPS.ANALYTICS_ENGINE ||
      active == MY_APPS.SCIENCE ||
      active == MY_APPS.API ||
      active == MY_APPS.DATA_QUALITY ||
      active == MY_APPS.TAG;
    const isProject =
      active == MY_APPS.RDOS ||
      active == MY_APPS.STREAM ||
              ...
              {hideDel ? (
                <span>
                  ...
                  <Popconfirm
                    ...
                    cancelText="取消"
                    onConfirm={() => {
                      ctx.removeUserFromProject(record);
                    }}>
      ...
      <div className="user-admin">
        ...
        <Modal
          ...
          width={
            Math.min(
              1000,
              Math.max(520, 400 + (roles ? roles.length * 60 : 0))
            ) + 'px'
          }>
          <EditMemberRoleForm
            myRoles={myRoles}
