...
import { Table, Card, DatePicker, Input, Select } from 'antd';
...
class AdminAudit extends React.Component<any, AdminAuditState> {
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    ...
    if (user !== prevProps.user) {
      // 非Root 用户重定向到首页
      if (user.id !== 0 && user.isRoot === false) {
        hashHistory.push('/');
      }
    }
  }
  ...
  renderTitle = () => {
    const { reqParams, active, operationList } = this.state;
    const timePicker = (
      <RangePicker
        size="default"
    ...
    const title = (
      <span>
        <RangePicker
          ...
          format="YYYY-MM-DD"
          placeholder={['开始时间', '结束时间']}
          style={{ float: 'left', width: '200px', margin: '0px 10px 0 0' }}
          onChange={this.onRangePickerChange}
        />
  ...
  render() {
    ...
    const finalApps = apps.filter(
      (app: any) => app.id == MY_APPS.RDOS || app.id == MY_APPS.API
    );
