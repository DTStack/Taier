...
export function updateComponentState(
  thisRef: { state: object; setState: Function },
  newState: object,
  callback?: Function
): void {
...
export function appendTreeNode(treeNode: any, append: any, target: any) {
  ...
  if (
    treeNode.id === targetId &&
    treeNode.children &&
    treeNode.type == target.type
  ) {
 ...
 * 字符串替换
 */
export function replaceStrFormBeginToEnd(
  str: any,
  replaceStr: any,
  begin: any,
  end: any
          ...
          <span>
            ...
            {notifyMsgs.length ? null : (
              <a
                ...
                className="dt-notification-close-btn">
                全部关闭
              </a>
            )}
          </span>
 ...
 */
export function generateValueDic(
  dic: any
): {
  [propName: number]: {
    symbol: any;
...
export function toRdosGateway(uri: any, params: any = {}) {
  ...
  const queryStr = keyAndValues
    .map(([key, value]) => {
      return `${key}=${value}`;
    })
    .join('&');
  window.open(
    `${rdosApp.link}gateway${queryStr ? `?${queryStr}` : ''}`,
    'rdos_open'
  );
}
...
export function isCurrentProjectChanged(key: any) {
  const projectIdCheckMap: any = {
    project_id: rdosApp.link.split('#')[0],
    science_project_id: scienceApp.link.split('#')[0],
    stream_project_id: streamApp.link.split('#')[0],
    tag_project_id: tagApp.link.split('#')[0],
    api_project_id: daApp.link.split('#')[0],
    dq_project_id: dqApp.link.split('#')[0],
  };
 ...
 */
export function compareEnableApp(
  apps: any,
  licenseApps: any,
  isShowHome?: any
) {
  ...
  if (licenseApps && licenseApps.length) {
    ...
    const notConigAppIds =
      validateAppIds.filter((id) => licenseAppIds.indexOf(id) === -1) || []; // 未返回的license Id
