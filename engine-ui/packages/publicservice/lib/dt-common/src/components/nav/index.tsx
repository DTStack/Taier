...
function renderMenuItem (menuItems: any, isRoot?: boolean, isRenderIcon = false) {
    return menuItems && menuItems.length > 0 ? menuItems.map((menu: any) => {
        const isShow = menu && menu.enable && (!menu.needRoot || (menu.needRoot && isRoot))
        return isShow ? (<Menu.Item key={menu.id} className={menu.menuClass}>
            ...
            {menu?.isDiffOrigin ? (
                ...
                <Link to={menu.link} target={menu.target} className="dropdown-content">
                    {(isRenderIcon || menu.enableIcon) && <span className={`iconfont icon-${menu.className || ''}`}></span>}
                    {menu.name}
                </Link>
            )}
 ...
 * @param menuItems 菜单项列表
 */
function renderATagMenuItem (menuItems: any, isRoot?: boolean, isRenderIcon = false) {
    return menuItems && menuItems.length > 0 ? menuItems.map((menu: any) => {
        const isShow = menu && menu.enable && (!menu.needRoot || (menu.needRoot && isRoot))
        return isShow ? (<Menu.Item key={menu.id} className={menu.menuClass}>
            <a href={menu.link} target={menu.target} className="dropdown-content">
...
function renderMenuItems (menuItems: any, isRoot?: boolean, isRenderIcon = false, subMenuList?: any) {
    ...
    let isShowSubMenu = subMenuItems && (subMenuItems.findIndex((item: any) => item && item.enable && (!item.needRoot || (item.needRoot && isRoot))) > -1)
    return (
        [ Menu,
    ...
    return (
        <div className="menu" style={{ flex: 1 }}>
            ...
            >
                ...
                {customItems.concat(renderMenuItems(menuItems, user.isRoot, false, subMenuItems))}
            </Menu>
        </div>
    )
}
        ...
        const settingMenuItems = (
            <Menu>
                <Menu.Item key="setting:1">
                    <Link to={`/admin/user${extraParms}`} target="blank" className='dropdown-content'>
                        <span className='iconfont icon-icon_usermanagement'></span>
                        用户管理
                    </Link>
                </Menu.Item>
                <Menu.Item key="setting:2">
                    <Link to={`/admin/role${extraParms}`} target="blank" className="dropdown-content">
        ...
        return (
            <div className="menu" style={{ height: '100%' }}>
                <menu className="menu-right">
                    ...
                    {isShowExt && isLeastOneLicAppShow && <Link to={`/message${extraParms}`} target="blank" style={{ color: '#ffffff' }}>
                        <span className="menu-item">
                            <Icon type="message" />
                        </span>
                    </Link>}
                    ...
                    <Dropdown overlayClassName='user_info_wrapper' overlay={userMenu} trigger={['click']} getPopupContainer={(triggerNode: any) => triggerNode.parentNode}>
