  ...
  wrapOption() {
    ...
    if (reactNodeArr && reactNodeArr.length > 0) {
      for (let i = 0; i < reactNodeArr.length; i++) {
        if (
          reactNodeArr[i].props.title
            ? reactNodeArr[i].props.title == searchValue
            : reactNodeArr[i].props.value == searchValue
        ) {
          return null;
        }
      }
    ...
    return searchValue ? (
      <Option key={searchValue} value={searchValue} title={searchValue}>
        {searchValue}
      </Option>
    ) : null;
  }
