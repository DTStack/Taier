...
const DynamicSelectList = (props: IPropsDynamicSelectList) => {
  ...
  useEffect(() => {
    if(!data) return;
    setRelationKeysList(data.map(item => ({
      id: id(),
  ...
  const deleteRelationList = (id: number) => {
    setRelationKeysList(relationKeysList => relationKeysList.filter(item => item.id !== id));
  }
              ...
              <Col offset={4} span={20}>
                <Row className="white-space-nowrap">
                    ...
                    <Form.Item wrapperCol={{ span: 24 }}>
                      {
                        getFieldDecorator(`relation-key-left_${index}`, {
                          ...
                          <Select placeholder="请选择">
                            <Select.Option key="aaa" value="aaa">aaa</Select.Option>
                            <Select.Option key="bbb" value="bbb">bbb</Select.Option>
                            <Select.Option key="ccc" value="ccc">ccc</Select.Option>
                            <Select.Option key="ddd" value="ddd">ddd</Select.Option>
                          </Select>
                        )
                      }
                    </Form.Item>
                  ...
                  <div className="operation">
                    <Icon className="icon icon-plus-circle" type="plus-circle" onClick={addRelationTable} />
                    {
                      index === 0 && relationKeysList.length <= 1 ? null : (
                        <Icon
                          className="icon icon-minus-circle"
                          type="minus-circle" onClick={() => deleteRelationList(item.id)}
                        />
                      )
                    }
                  </div>
