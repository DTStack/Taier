...
import {
	...
	validateFlinkParams,
	validateHiveParams,
	validateCarbonDataParams,
	validateSparkParams,
	validateDtYarnShellParams,
	validateLearningParams,
	validateHiveServerParams,
	validateLibraParams,
	validateSftpDataParams,
	validateImpalaSqlParams,
	RESOURCE_TYPE,
} from './index';
 ...
 */
export function exChangeComponentConf(
	hadoopComp: any = [],
	libraComp: any = [],
	tidbComp: any = [],
	oracleComp: any = [],
	...
	tabCompData &&
		tabCompData.map((item: any) => {
			validateFieldsAndScroll(
				validateCompParams(item.componentTypeCode),
				{
					force: true,
					scroll: {
						offsetBottom: 150,
				...
				(err: any, values: any) => {
					...
					if (item.componentTypeCode == COMPONENT_TYPE_VALUE.FLINK) {
						...
						item.componentTypeCode === COMPONENT_TYPE_VALUE.SPARK_THRIFT_SERVER
					) {
						if (!err) {
							obj = Object.assign(obj, {
