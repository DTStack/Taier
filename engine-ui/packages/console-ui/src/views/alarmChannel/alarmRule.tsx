...
const AlarmRule: React.FC = (props: any) => {
	...
	const testAlarm = () => {
		validateFields(async (err, values) => {
			if (!err) {
				...
				let testValue =
					values.alertGateType !== ALARM_TYPE.CUSTOM ? values[testKey].split(',') : '';
				let res = await Api.testAlert(
	...
	const handleSubmit = () => {
		validateFields(async (err, values) => {
			if (!err) {
				let res = await Api.addOrUpdateAlarmRule(
					Object.assign({}, values, {
						id: id || '',
						isDefault: values.isDefault ? 1 : 0,
						file: values?.file?.file ?? values?.file ?? '',
	...
	return (
		<div className="alarm-rule__wrapper">
			<Breadcrumb>
				...
				<Breadcrumb.Item>{`${
					isCreate ? '新增告警通道' : `${ruleData?.alertGateName}`
				}`}</Breadcrumb.Item>
			</Breadcrumb>
			<Card bordered={false}>
				<Form>
					<FormItem {...{ ...formItemCenterLayout, ...wrapperCol }} label="通道类型">
					...
					{showAlertGateCode(alertGateType) && (
						<FormItem {...formItemCenterLayout} label="通道模式">
							...
							})(<Select>{getChannelModeOpts()}</Select>)}
						</FormItem>
					)}
					{alertGateCode?.includes('jar') && !showConfigFile(alertGateType) ? (
						<FormItem {...formItemCenterLayout} label="上传文件">
							...
							{ruleData?.filePath && !getFieldValue('file') && (
								<span>{ruleData?.filePath}</span>
							)}
						</FormItem>
					) : null}
					...
					{showConfigFile(alertGateType) && (
						<FormItem {...{ ...formItemCenterLayout, ...wrapperCol }} label="配置文件">
							{getFieldDecorator('file', {
								rules: [
									{
										required: true,
							...
							<div className="c-alarmRule__config">
								<Upload {...uploadConfigProp}>
									<Button style={{ width: 164 }} icon="upload">
										点击上传
									</Button>
								</Upload>
								<span className="config-desc">
									仅支持jar格式，
									<a
										href={`/api/console/service/alert/downloadJar?alertGateType=${alertGateType}`}
									>
										查看配置文件说明
									</a>
								</span>
					...
					{showAlertGateJson(alertGateCode, alertGateType) ? (
					...
					{canTestAlarm(alertGateType) ? (
						<FormItem {...formItemCenterLayout} label=" " colon={false}>
							{getFieldDecorator(`${alertKey}`, {
								...
								alertGateType !== ALARM_TYPE.CUSTOM ? (
									<Input
