...
class QueueManage extends React.Component<any, any> {
	...
	getClusterOptionView() {
		...
		return clusterList.map((item: any, index: any) => {
			return (
				<Option key={item.id} value={`${item.id}`} data-item={item}>
					{item.clusterName}
				</Option>
			);
		});
		...
		Modal.confirm({
			...
			content: (
				<div style={{ fontSize: '14px' }}>
					<p>
						杀死所有<RedTxt>队列中、已存储、等待重试、等待资源</RedTxt>的任务
					</p>
					<p>
						<RedTxt>运行中的任务不会被杀死</RedTxt>
						，可点击运行中的任务，并执行批量杀死操作
					</p>
				</div>
			),
	...
	initTableColumns() {
		...
		return [
			...
			},
			{
				...
				render(text: any, record: any) {
					const txt =
						text + (record.priorityWaitTime ? ` (${record.priorityWaitTime})` : '');
	...
	render() {
		...
		const { dataSource, table, clusterId, node } = this.state;
		const { loading } = table;
		...
		return (
			...
			>
				...
				<Card style={{ marginTop: '0px' }} className="box-1" hoverable>
					<Table
						rowKey={(record: any) => {
