					...
					return (
						<div>
							...
							{stickTxt ? (
								<span>
									<span className="ant-divider"></span>
									<a onClick={this.stickTask.bind(this, record, index)}>
										{stickTxt}
									</a>
								</span>
							) : null}
	...
	tableFooter = () => {
		...
		const menu = (
			<Menu onClick={this.handleKillAll}>
				<Menu.Item key="1" style={{ width: 118 }}>
					杀死全部任务
				</Menu.Item>
			</Menu>
		);
			...
			<Row style={{ width: '100%' }}>
				<Col span={12}>
					...
					<span style={{ marginLeft: 8, color: '#666' }}>
						已选中<a>{selectedRowKeys.length}</a>条
					</span>
				</Col>
				<Col span={12}>
					<Pagination
				...
				<div className={`${className}__title`}>
					<Breadcrumb>
						<Breadcrumb.Item>
							<a
								onClick={() => {
									this.props.router.push('/console-ui/queueManage');
								}}
							>
								队列管理
							</a>
				...
				<div className={`${className}__radioWrap`}>
					...
					>
						{Object.getOwnPropertyNames(JobStageText).map((statusValue) => (
							<RadioButton key={statusValue} value={statusValue}>
								{JobStageText[statusValue]}
							</RadioButton>
						))}
					</RadioGroup>
					<Tooltip title="刷新数据">
