...
import { Form, Breadcrumb, Tabs, Button, message, Modal, Icon } from 'antd';
...
class EditCluster extends React.Component<any, IState> {
	...
	getLoadTemplate = async (key?: string, params?: any) => {
		...
		if (
			(!comp?.componentTemplate && initialCompData[activeKey]?.length) ||
			params?.compVersion ||
			params?.storeType
		) {
	...
	handleComplete = () => {
		...
		const showConfirm = (arr: any[]) => {
			confirm({
				...
				content: null,
				icon: (
					<Icon style={{ color: '#FAAD14' }} type="exclamation-circle" theme="filled" />
				),
				okText: '保存',
				cancelText: '取消',
				onOk: () => {},
	...
	setTestStatus = (status: any, isSingle?: boolean) => {
		if (isSingle) {
			...
			const currentComp = initialCompData[activeKey].find(
	...
	testConnects = (params?: any, callBack?: Function) => {
		...
		form.validateFields(null, {}, (err: any, values: any) => {
			...
			if (
				(err && !typeCode) ||
				(err && !isMultiVersion(typeCode) && Object.keys(err).includes(String(typeCode)))
			) {
				message.error('请检查配置');
				return;
			}
			...
			if (typeCode || typeCode === 0) {
				if (
					modifyComps.size > 0 &&
					includesCurrentComp(Array.from(modifyComps), { typeCode, hadoopVersion })
				) {
			...
			<div className="c-editCluster__containerWrap">
				...
				<div className="c-editCluster__container">
					<Tabs
						...
						tabBarExtraContent={
							<div className="c-editCluster__commonTabs__title">集群配置</div>
						}
					>
							...
							return (
								...
								>
									...
									<Tabs
										...
										onChange={(key: any) => {
											if (!isMultiVersion(Number(key)))
												this.getLoadTemplate(key);
										}}
									>
										{comps?.length > 0 &&
											comps.map((comp: any) => {
