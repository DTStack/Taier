...
export default class FileConfig extends React.PureComponent<IProps, IState> {
	...
	renderCompsVersion = () => {
		...
		const typeCode = comp?.componentTypeCode ?? '';
		let version = isOtherVersion(typeCode)
	...
	downloadFile = (type: number) => {
		...
		param =
			param +
			`type=${type}&componentType=${typeCode}&hadoopVersion=${version}&deployType=${deployType}&clusterName=${clusterInfo?.clusterName}`;
			...
			<UploadFile
				label={
					<span>
						配置文件
						{isYarn(typeCode) && (
							<a style={{ marginLeft: 66 }} onClick={this.refreshYarnQueue}>
								刷新队列
							</a>
						)}
					</span>
	...
	renderStorageComponents = () => {
		...
		return (
			<FormItem label="存储组件" colon={false} key={formField}>
	...
	renderPrincipal = () => {
		...
		return (
			<FormItem label="principal" colon={false} key={formField + '.principal'}>
