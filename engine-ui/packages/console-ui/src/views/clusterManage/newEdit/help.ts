...
export function getCompsName(comps: any[]): any[] {
	return Array.from(comps).map((comp: any) => {
		if (isMultiVersion(comp.typeCode)) {
			return (
				COMPONENT_CONFIG_NAME[comp.typeCode] +
				' ' +
				(Number(comp.hadoopVersion) / 100).toFixed(2)
			);
		}
...
export function handleComponentTemplate(comp: any, initialCompData: any): any {
	/** 外层数据先删除一层自定义参数 */
	let newComponentTemplate = JSON.parse(initialCompData.componentTemplate).filter(
		(v) => v.type !== CONFIG_ITEM_TYPE.CUSTOM_CONTROL,
	);
				...
				(isDeployMode(newComponentTemplate[0].key)
					...
					if (temps.key == key) {
						temps.values = temps.values.filter(
							(temp) => temp.type !== CONFIG_ITEM_TYPE.CUSTOM_CONTROL,
						);
