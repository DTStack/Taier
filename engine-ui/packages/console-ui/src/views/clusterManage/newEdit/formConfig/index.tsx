...
export default class FormConfig extends React.PureComponent<IProps, any> {
				...
				return (
					<RadioGroup disabled={view}>
						{temp.values.map((comp: any) => {
							return (
								<Radio key={comp.key} value={comp.value}>
									{comp.key}
								</Radio>
							);
						})}
			...
			case CONFIG_ITEM_TYPE.SELECT:
				return (
					<Select disabled={view} style={{ width: 200 }}>
						{temp.values.map((comp: any) => {
							return (
								<Option key={comp.key} value={comp.value}>
									{comp.key}
								</Option>
							);
						})}
			...
			case CONFIG_ITEM_TYPE.CHECKBOX:
				return (
					<CheckboxGroup disabled={view} className="c-componentConfig__checkboxGroup">
						{temp.values.map((comp: any) => {
							return (
								<Checkbox key={comp.key} value={`${comp.value}`}>
									{comp.key}
								</Checkbox>
							);
						})}
			...
			if (isDeployMode(temps.key) || isRadioLinkage(temps.type)) {
				return (
					<>
						...
						{temps.values.map((temp: any) =>
							this.renderGroupConfigItem(temp, isRadioLinkage(temps.type)),
						)}
					</>
				);
	...
	render() {
		...
		return <div className={className}>{this.renderComponentsConfig()}</div>;
	}
}
