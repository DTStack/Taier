...
export default class MultiVersionComp extends React.Component<IProps, IState> {
			...
			<Menu onClick={this.handleMenuClick}>
				{displayVersion?.map(({ value }) => {
					const disabled = comp?.multiVersion?.findIndex(
						(vcomp) => vcomp.hadoopVersion == value,
					);
					return (
	...
	render() {
		...
		if (isFLink(typeCode)) {
			displayVersion = displayVersion.filter((item) =>
				item?.deployTypes?.includes(deployType),
			);
		}
									...
									return (
										...
										>
											<span className="comp-name">
												<img
													src={`public/img/${VERSION_TYPE[typeCode]}.png`}
												/>
												<span>
													{!isFLink(typeCode) &&
														COMPONENT_CONFIG_NAME[typeCode] + ' '}
			...
			<div className={className}>
				<Tabs
					...
					tabBarExtraContent={
						<Dropdown
							disabled={view}
							overlay={this.getMeunItem(displayVersion)}
					...
					{comp?.multiVersion.map((vcomp) => {
						...
						return (
							<TabPane
								tab={
									<span>
										{this.getComponentName(componentTypeCode, deployType)}{' '}
										{this.getCompVersion(hadoopVersion)}
										<TestRestIcon
