...
import { Table, message, Select, Form, Checkbox, Tabs, Pagination, Col, Button } from 'antd';
...
class OfflineTaskMana extends React.Component<any, any> {
	...
	onSearchByTaskName = () => {
		this.setState({ current: 1 }, this.search);
	};
				...
				render: (text: any, record: any) => {
					const content =
						record.isDeleted === 1 ? (
							...
							>
								{record.name +
									(record.scheduleStatus == SCHEDULE_STATUS.STOPPED
										? ' (已冻结)'
										: '')}
							</a>
						);
