...
class OfflineTaskList extends React.Component<any, any> {
	state: any = {
		...
		taskStatus: isEmpty(utils.getParameterByName('status'))
			? []
			: utils.getParameterByName('status').split(','),
		bussinessDate: [
			moment(yesterDay).subtract(utils.getParameterByName('date') || 0, 'days'),
			yesterDay,
		],
		cycDate: undefined,
		checkAll: false,
		...
		Api.getTaskTypesX(parmms).then((res: any) => {
			if (res.code === 1) {
				...
				let taskTypeFilter =
					taskTypes &&
					taskTypes
						.filter((item) => item.value === 'Shell' || item.value === 'ImpalaSQL')
						.map((type: any) => {
							return {
								value: type.key,
								id: type.key,
						...
						<span>
							...
							{record.isDirty && text == TASK_STATUS.FINISHED ? (
								<Tooltip title="部分数据未同步成功，建议检查配置">
									<Icon
										type="info-circle-o"
										style={{ color: '#ee9b1e', marginLeft: '5px' }}
									/>
								</Tooltip>
							) : null}
						</span>
				...
				if (res.code == 1) {
					let newData = newTasks.data.map((task: any) => {
						if (
							...
							task?.children.some((item: any) => item.jobId == jobId)
						) {
				...
				<Form layout="inline" style={{ marginBottom: 12 }}>
					<Row>
						<Col span={23}>
							<FormItem
								label=""
								className="batch-operation_offlineImg dt-form-shadow-bg"
							>
								<MultiSearchInput
									placeholder="按任务名称搜索"
									value={jobName}
							...
							<FormItem label={getTitle('业务日期')}>
								<div className="m-date-margin">
									<RangePicker
										...
										ranges={{
											昨天: [
												(moment() as any).subtract(2, 'days'),
												yesterDay,
											],
											最近7天: [
												(moment() as any).subtract(8, 'days'),
												yesterDay,
											],
											最近30天: [
												(moment() as any).subtract(31, 'days'),
												yesterDay,
											],
										}}
								...
								<div className="m-date-margin">
									<RangePicker
										...
										ranges={{
											...
											最近7天: [
												(moment() as any).subtract(7, 'days'),
												moment() as any,
											],
											最近30天: [
												(moment() as any).subtract(30, 'days'),
												moment() as any,
											],
										}}
										value={cycDate || null}
					...
					<Table
						...
						rowClassName={(record: any, index: any) => {
							if (
								...
								this.state.selectedTask.id == record.id
							) {
								return 'row-select';
							} else {
								return '';
