...
import { Tooltip, Spin, Icon, Card } from 'antd';
...
import { goToTaskDev, removeToolTips } from '../hlep';
...
class TaskGraphView extends React.Component<any, any> {
	...
	initGraph = async (graphData: any) => {
		...
		const res: any =
			graphData?.taskId && graphData?.appType
				? await Api.findTaskRuleTask({
						appType: graphData?.appType,
						taskId: graphData?.taskId,
				  })
				: { data: {} };
		...
		const getVertex = (parentCell: any, data: any) => {
			...
			if (isWorkflow) {
				...
				graph.insertVertex(
					...
					50,
					VertexSize.width,
					VertexSize.height, // geo.width, geo.height,
					style,
				);
				/* eslint-disable-next-line */
									...
									return (
										<p
											...
											style={{
												...
												marginBottom:
													index <
													currentInfo.scheduleDetailsVOList.length - 1
														? 12
														: 0,
											}}
										>
					...
					<>
						...
						>
							...
							<span style={{ marginLeft: '15px' }}>
								{get(data, 'createUser.userName', '-')}
							</span>
							&nbsp; 发布于
							<span>{utils.formatDateTime(get(data, 'gmtModified'))}</span>&nbsp;
							{APP_TYPE[data.appType] === APP_TYPE[1] && (
								<a
									onClick={() => {
										goToTaskDev(data);
									}}
