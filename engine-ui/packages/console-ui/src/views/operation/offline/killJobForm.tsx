...
class KillJobForm extends React.Component<any, any> {
	...
	handleSechedulingChange = (checkedList: any) => {
		const indeterminate =
			!!checkedList.length && checkedList.length < this.getSchedulingOptions().length;
		...
		validateFields((err: any, values: any) => {
			if (!err) {
				...
				Api.batchStopJobByDate({
					...
					taskPeriodId: values.schedulingCycle
						? values.schedulingCycle.join(',')
						: undefined,
	...
	render() {
		...
		return (
			...
			>
				...
				/>
				<Form>
					<FormItem {...formItemLayout} label="选择任务">
								...
								<Row>
									...
									<Col span={20}>
										{getFieldDecorator('schedulingCycle', {
											...
											<CheckboxGroup
												options={this.getSchedulingOptions()}
												onChange={this.handleSechedulingChange}
											/>,
										)}
									</Col>
								</Row>
