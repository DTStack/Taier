...
class PatchData extends React.Component<any, any> {
	...
	addData = (prevProps: any) => {
		...
		this.props.form.validateFields((err: any) => {
			if (!err) {
				...
				reqParams.fromDay = reqParams.rangeDate[0]
					.set({
						hour: 0,
						minute: 0,
						second: 0,
					...
					.unix();
				reqParams.toDay = reqParams.rangeDate[1]
					.set({
						hour: 23,
						minute: 59,
						second: 59,
		...
		const loop = (data: any, tree: any) => {
			for (let i = 0; i < data.length; i++) {
				...
				if (result) {
					...
					if (
						result &&
						Array.isArray(data[i].taskRuleList) &&
						data[i].taskRuleList.length > 0
					) {
				...
				if (data[i].subTaskVOS && _checkedKeys.length !== 0) {
					loop(
						data[i].subTaskVOS,
						result && tree.length !== 0 ? tree[tree.length - 1].children : tree,
					);
				}
			}
		};
	...
	onCheck = (checkedKeys: any, checkedNodes: any) => {
		...
		function getPath(
			...
			visitTree(tree, (node, level) => {
				...
				if (node.key.indexOf(key) == 0) {
					...
					const lastLevel =
						lastPath && lastPath.length ? lastPath[lastPath.length - 1].level : null;
					if (lastLevel == null || lastLevel >= level) {
						const newRootPath = (lastPath || [])
							.map((item) => {
								return item.level < level ? item : null;
							})
							.filter(Boolean);
		...
		function remove(key: any, tree: any, result: any) {
			...
			let { children, parents } = getPath(tree, key);
			let haveParent =
				new Set(parents.concat(checked)).size !== parents.length + checked.length;
			if (haveParent) {
	...
	getTreeNodes = (data: any) => {
		if (data && data.length > 0) {
			const nodes = data.map((item: any) => {
				...
				if (item.subTaskVOS) {
					return (
						<TreeNode data={item} value={itemValue} title={content} key={itemValue}>
	...
	expandChange(expandedKeys: any, expand: any) {
		if (!expand.expanded) {
			...
			expandedKeys = expandedKeys.filter((item: any) => {
								...
								<Col span={11}>
									{getFieldDecorator('concreteStartTime', {
										...
										<TimePicker
											...
											disabledMinutes={this.disabledMinutes.bind(
												this,
												'start',
											)}
