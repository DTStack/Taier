...
class TaskJobFlowView extends React.Component<any, any> {
    ...
    componentWillReceiveProps(nextProps: any) {
        ...
        if (
            taskJob &&
            visibleSlidePane &&
            (!currentJob || taskJob.id !== currentJob.id)
        ) {
                    ...
                    ? ctx.setState(
                        ...
                        () => {
                            ctx.state.frontPeriodsList.map((item: any) => {
                                const times = moment(item.cycTime).format(
                                    'YYYY-MM-DD HH:mm:ss'
                                );
    ...
    initGraphEvent = (graph: any) => {
        ...
        if (graph) {
            ...
            graph.addListener(mxEvent.CELLS_FOLDED, function (sender: any, evt: any) {
                ...
                const isWorkflow =
                    get(cell, 'data.batchTask.taskType') === TASK_TYPE.WORKFLOW;
            ...
            graph.addListener(mxEvent.CLICK, async function (sender: any, evt: any) {
                ...
                if (cell && which === 1) {
                    const jobId = cell?.data?.jobId;
                    if (jobId) {
                        let res = await Api.findTaskRuleJob({ jobId });
                        ctx.setState({
        ...
        return (
            ...
            >
                ...
                <Modal
                    ...
                    onCancel={() => {
                        this.setState({ logVisible: false, logPage: { current: 0 } });
                    }}
                    footer={null}
                    maskClosable={true}
                >
                    <LogInfo
                        log={taskLog.logInfo}
                        // log={taskLog.engineLog||taskLog.logInfo}
