...
class PatchDataDetail extends React.Component<any, any> {
	...
	render() {
		...
		const tableFooter = (currentPageData: any) => {
			...
			const couldKill =
				selectStatus.haveRunning &&
				!selectStatus.haveFail &&
				!selectStatus.haveNotRun &&
				!selectStatus.haveFail;
			const couldReRun =
				!selectStatus.haveRunning &&
				(selectStatus.haveSuccess ||
					selectStatus.haveFail ||
					selectStatus.haveNotRun ||
					selectStatus.haveFail);
			return (
						...
						<tr className="ant-table-row  ant-table-row-level-0">
							<td>
								...
								<Button
									...
									disabled={!couldReRun}
									type="primary"
									onClick={this.batchReloadJobs}
								>
									重跑当前及下游任务
								</Button>
								&nbsp;
								<Button type="primary" onClick={this.killAllJobs}>
									杀死所有实例
								</Button>
								&nbsp;
							</td>
		...
		return (
			<div>
				...
				<div className="box-2 m-card task-manage" style={{ minWidth: '1167px' }}>
					<Card hoverable bordered={false} loading={false}>
						<div style={{ margin: '16px 10px' }}>
							<span className="ope-statistics">{this.renderStatus(statusList)}</span>
						</div>
						<Table
							rowKey="id"
