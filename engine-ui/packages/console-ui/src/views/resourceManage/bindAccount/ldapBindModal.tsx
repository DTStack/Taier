...
import { Modal, Alert, Table, Select, Input, Icon, Form } from 'antd';
				...
				render: (text: any, record: TableData) => {
					return (
						<Select
							...
							style={{ width: 160 }}
							onChange={(value) =>
								this.handleValueChange('bindUserId', value, record)
							}
						>
							{userList.map((user) => {
								return (
					...
					{!data ? (
						<React.Fragment>
							<Table
								...
								pagination={false}
							/>
							{tableData.length > MAX_ACCOUNT_NUM ? (
								<a style={{ color: '#BFBFBF' }}>
									<Icon type="plus" />
									新增
								</a>
							) : (
								<a style={{ color: '#3F87FF' }} onClick={this.addTableData}>
									<Icon type="plus" />
									新增
								</a>
							)}
						...
						<React.Fragment>
							<Form>
								<Form.Item key="bindUserId" label="产品账号" {...formItemLayout}>
