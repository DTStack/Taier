...
class BindAccountTable extends React.Component<IProps, IState> {
	...
	fetchUnbindUsers = async () => {
		const { queryParams } = this.state;
		const { engineType } = this.props;
		const res = await AccountApi.getUnbindAccounts({
	...
	render() {
		const { tableData, queryParams, loading, visible, modalData, tenantList, unbindUserList } =
			this.state;
		const { engineType } = this.props;
		...
		return (
			<div style={{ padding: 20 }}>
				...
				<Search
					...
					placeholder={`按产品账号、${
						isHadoopEngine(engineType) ? 'LDAP' : '数据库'
					}账号搜索`}
				...
				<span style={{ marginLeft: 10 }}>
					<Button type="primary" onClick={() => this.showHideBindModal()}>
						绑定账号
					</Button>
				</span>
				<Table
