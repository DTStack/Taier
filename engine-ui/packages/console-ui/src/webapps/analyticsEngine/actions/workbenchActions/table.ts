 ...
 * @param {导航数据} params
 */
export function onCreateTable (params: any) {
    console.log(params)
    return (dispatch: any, getStore: any) => {
        ...
        const newCreateTableTabData: any = {
            id: moment().unix(),
...
export function onEditTable (params: any) {
    console.log(params)
    return async (dispatch: any, getStore: any) => {
        ...
        if (res.code === 1) {
            res.data.columns.map((o: any, i: any) => {
                o._fid = i
            })
            const newEditTableTabData: any = {
                id: moment().unix(),
...
* 保存新建表数据到storage
*/
export function saveNewTableData (params: any) {
    console.log(params)
    return (dispatch: any) => {
        dispatch({
 ...
 * 获取表详情
 */
export function onTableDetail (params: any) {
    return async (dispatch: any, getStore: any) => {
        const { workbench } = getStore();
        const { tabs } = workbench.mainBench;
        const res = await API.getTableById({
...
export function handleSave () {
    return async (dispatch: any, getStore: any) => {
        ...
        params.partitions.columns.map((o: any) => {
            if (o.name === '' || o.type === '') {
                notification.error({
                    message: '提示',
 ...
 */
export function saveEditTableInfo (params: any) {
    return (dispatch: any) => {
        return dispatch({
 ...
 * @param {预留} param
 */
export function saveTableInfo (param: any) {
    return async (dispatch: any, getStore: any) => {
        const { workbench } = getStore();
        const { tabs, currentTab } = workbench.mainBench;
        let tableDetail: any = {};
 ...
 * @param {databaseId, id}
 */
export function toTableDetail (params: any) {
    return async (dispatch: any, getStore: any) => {
        const { workbench } = getStore();
        const { tabs, currentTab } = workbench.mainBench;
