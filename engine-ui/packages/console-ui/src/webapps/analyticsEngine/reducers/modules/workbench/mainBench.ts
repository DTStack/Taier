...
export default function mainBench (state = getInitialCachedData(), action: any) {
    ...
    switch (type) {
        ...
        case workbenchAction.OPEN_TAB: {
            if (payload) {
                ...
                const isExist = tabs.find((item: any) => item.id === payload.id);
                if (!isExist) {
                    tabs.push(payload);
                }
                tabs.map((o: any, i: any) => {
                    if (o.id === payload.id) { tabs[i] = payload }
                })
        ...
        case workbenchAction.CLOSE_OTHERS: {
            ...
            newStore.tabs = newStore.tabs.filter((item: any) => {
        ...
        case workbenchAction.UPDATE_TAB: {
            if (payload) {
                const tabs = [...state.tabs];
                const index = tabs.findIndex((item: any) => item.id === payload.id);
                if (index > -1) {
