                  ...
                  <Select defaultValue={record.flagIndex} style={{ width: 159 }} onChange={(e) => this.handleBucketChange(e, record)}>
                      {
                          this.state.columns.map(o => {
                              if (o.name && bucketType.indexOf(o.type) !== -1) { return <Option key={o._fid} value={this.state.columns.indexOf(o)}>{o.name}</Option> }
                          })
                      }
                  </Select>
              )
      ...
      let colField = [
          {
            ...
            <span>
              ...
              dataIndex: 'invert',
              render: (text, record) => (
                  <Checkbox defaultChecked={text === 1} onChange={(e) => this.handleInvert(e, record)}></Checkbox>
              )
          }, {
              title: '字典编码',
