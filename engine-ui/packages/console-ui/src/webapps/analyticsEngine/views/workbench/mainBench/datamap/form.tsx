...
class DataMapForm extends React.Component<any, any> {
    ...
    dynamicRender = (timestampColumns: any) => {
        ...
        const timeColumnsOptions = tableColumns && tableColumns.map((opt: any) => (
            <Option key={`${opt.name}`} value={`${opt.name}`}>{opt.name}</Option>
        ))
        const timeStringColumns = tableColumns && tableColumns.filter((item: any) => {
            return item.type == 'string'
        })
        const timeStringColumnsOptions = timeStringColumns && timeStringColumns.map((opt: any) => (
            <Option key={`${opt.name}`} value={`${opt.name}`}>{opt.name}</Option>
        ))
        const timeColumnsOptionsForSeq = timestampColumns && timestampColumns.map((opt: any) =>
            <Option key={`${opt.name}`} value={`${opt.name}`}>{opt.name}</Option>
        )
    ...
    render () {
        ...
        const timestampColumns = tableColumns.filter((item: any) => item.type === 'timestamp');
...
const FormWrapper = Form.create<any>()(DataMapForm);
