 ...
 * 获取参数位置的select组件
 */
export function getPositionSelect (hideBody: any) {
    return (
        <Select style={{ width: '100%' }}>
...
export function getCommonDelete (props: any, record: any) {
    return <Popconfirm
        title="确认删除吗？"
        onConfirm={() => {
 ...
 * @param {*} type 输入输出参数
 */
export function generateFormItemKey (itemName: any, id: any) {
 ...
 * 动态表单通用方法
 */
export function mapPropsToFields (props: any) {
    const { data } = props;
    let formValues: any = {};
    /**
     * 遍历每一个属性，加上form属性映射
 ...
 * 动态表单通用方法
 */
export function onValuesChange (props: any, values: any) {
    props.updateColumnData(values);
}

export const formItemParamOption: any = {
    rules: [
        {
            required: true,
            message: '请输入参数名称'
