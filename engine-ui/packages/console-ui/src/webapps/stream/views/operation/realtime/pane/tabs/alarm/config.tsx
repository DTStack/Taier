...
class AlarmConfigList extends React.Component<any, any> {
    state: any = {
        pagination: {
            total: 0,
            pageSize: 5,
    ...
    loadAlarmRules (data: any) {
        ...
        Api.getAlarmList(reqForm).then((res: any) => {
            this.setState({
                loading: false
            })
            if (res.code == 1) {
        ...
        return [{
            ...
            render: (text: any, record: any) => {
                ...
                if (recivers.length > 0) {
                    return recivers.map((item: any) => <span key={record.alarmId}>{item.userName};</span>)
                }
                return ''
            }
        }, {
    ...
    addAlarm = (alarm: any) => {
        ...
        return Api.addAlarm(alarm).then((res: any) => {
            if (res.code === 1) {
    ...
    deleteAlarm (alarm: any) {
        const ctx = this
        Api.deleteAlarm(alarm).then((res: any) => {
            if (res.code === 1) {
    ...
    updateAlarmStatus (alarm: any) {
        ...
        if (alarm.alarmStatus === 0) {
            Api.closeAlarm(params).then((res: any) => {
                if (res.code === 1) {
            ...
            Api.openAlarm(params).then((res: any) => {
                if (res.code === 1) {
    ...
    updateAlarm = (alarm: any) => {
        ...
        return Api.updateAlarm(alarm).then((res: any) => {
            if (res.code === 1) {
