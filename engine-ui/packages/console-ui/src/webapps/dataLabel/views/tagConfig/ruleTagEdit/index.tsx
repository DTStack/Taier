...
class RuleTagEdit extends React.Component<any, any> {
    state: any = {
    ...
    // 获取标签详细数据
    getTagDetailInfo = () => {
        TCApi.getRuleTagDetail({ tagId: this.state.tagId }).then((res: any) => {
            if (res.code === 1) {
                this.setState({
    ...
    getComputeSourceData = () => {
        TCApi.getComputeSource().then((res: any) => {
            if (res.code === 1) {
                this.setState({
    ...
    initColumns = () => {
        return [{
            ...
            key: 'dataSourceId',
            width: '25%',
            render: (text: any, record: any) => this.renderColumns(text, record, 'dataSourceId')
        }, {
            title: '表名',
            dataIndex: 'tableName',
            key: 'tableName',
            width: '25%',
            render: (text: any, record: any) => this.renderColumns(text, record, 'tableName')
        }, {
            title: '过滤条件',
            dataIndex: 'sqlCondition',
            key: 'sqlCondition',
            width: '40%',
            render: (text: any, record: any) => this.renderColumns(text, record, 'sqlCondition')
        }, {
            title: '操作',
            width: '10%',
    ...
    saveCondition(id) {
        ...
        form.validateFields(['dataSourceId', 'tableName', 'sqlCondition'], (err: any, values: any) => {
            if (!err) {
                delete curCondition.editable;

                let source = [...sourceList].filter((item: any) => item.id == values.dataSourceId)[0];
    ...
    saveRuleTag = () => {
        ...
        form.validateFields((err: any, values: any) => {
            ...
            if (!err) {
                // 新增的condition不需要id
                conditionList.forEach((element: any, index: any) => {
                        ...
                        <FormItem {...formItemLayout} label="通知方式">
                            {
                                getFieldDecorator('sendTypes', {
                                    ...
                                    initialValue: sendTypes.map((item: any) => item.toString())
                                })(
                                    <Checkbox.Group onChange={this.onSendTypesChange}>
                                        {
                                            notifyType.map((item: any) => {
                                                return <Checkbox
                                                    key={item.value}
                                                    value={item.value.toString()}>
                                                    {item.name}
                        ...
                        <FormItem {...formItemLayout} label="通知接收人">
                            {
                                getFieldDecorator('receivers', {
                                    ...
                                    initialValue: receivers.map((item: any) => item.toString())
                                })(
                                    <Select
                                        allowClear
                                        mode="multiple"
...
export default (Form.create<any>()(RuleTagEdit));