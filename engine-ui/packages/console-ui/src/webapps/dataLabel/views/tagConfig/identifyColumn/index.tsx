...
class IdentifyColumn extends React.Component<any, any> {
    state: any = {
        visible: false,
        loading: false,
        queryParams: {
    ...
    // 获取识别列数据
    getColumnData = (params: any) => {
        this.setState({ loading: true });

        TCApi.queryIdentifyColumn(params).then((res: any) => {
            if (res.code === 1) {
                this.setState({
                    columnData: res.data,
                    loading: false
    ...
    deleteColumn = (id: any) => {
        ...
        if (id) {
            TCApi.deleteIdentifyColumn({ identifyId: id }).then((res: any) => {
                if (res.code === 1) {
...
export default (Form.create<any>()(IdentifyColumn));
