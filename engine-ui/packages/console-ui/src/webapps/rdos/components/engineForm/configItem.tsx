...
class EngineConfigItem extends React.Component<any, any> {
    state: any = {
        visible: false,
        dbName: ''
    }
    ...
    renderDbOptions = (dataBase: any) => {
        return dataBase.map((item: any) => {
            return <Option key={`${item}`} value={item}>{item}</Option>
        })
    }
    onPreviewMetaData (engineType: any, dbName: any) {
        if (dbName) {
            this.setState({
                visible: true,
                dbName,
