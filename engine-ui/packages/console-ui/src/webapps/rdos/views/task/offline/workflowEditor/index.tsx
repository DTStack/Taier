...
const getTaskBaseData = (task: any) => {
    return {
        id: task.id,
        name: task.name,
        type: task.type,
...
class WorkflowEditor extends React.Component<any, any> {
        ...
        const editSucc = (evt: any) => {
            ...
            if ((evt.type === 'keypress' && (event as any).keyCode === 13) || evt.type === 'blur') {
                ...
                if (checkNodeName(value) && value !== originName) {
                    ...
                    API.renameTask({ taskId: task.id, taskName: value }).then((res: any) => {
                        if (res.code === 1) {
    ...
    appendWorkflowNode = (newNode: any) => {
    ...
    deleteTask = (cell: any) => {
        ...
        if (taskData && cell.vertex) {
            Modal.confirm({
                ...
                onOk () {
                    delOfflineTask({ taskId: taskData.id }, data.id).then((res: any) => {
    ...
    updateCellData = (cell: any, cellData: any) => {
        if (cell) {
    ...
    initGraphData = (cells: any) => {
        const { tabs, updateTabData, data } = this.props;
        const waitUpdateTabs: any = [];
        if (cells) {
            for (let i = 0; i < cells.length; i++) {
                const cell = cells[i];
                if (cell.vertex && cell.data) {
                    const item = tabs.find((i: any) => i.id === cell.data.id)
                    if (item) {
    ...
    initDraggableToolBar () {
        ...
        for (let i = 0; i < taskTypes.length; i++) {
            ...
            if (dragTarget) {
                ...
                draggabledEle.createPreviewElement = function (graph: any, event: any) {
    ...
    updateGraphData = () => {
        ...
        const toUpdateTasks = workflow.filter((item: any) => {
    ...
    renderToolBar = () => {
        ...
        const showTitle = (type: any, title: any) => {
            switch (type) {
                case TASK_TYPE.SQL:
        ...
        const widgets = taskTypes.map((item: any) => {
