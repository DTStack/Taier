...
export default class TaskVersion extends React.Component<any, any> {
    state: any = {
        showDiff: false,
        haveCode: false,
        compareTo: '',
     ...
     * 加载调度配置
     */
    getScheduleConfData = (id: any) => {
        this.setState({
            historyValue: {}
        })
        ajax.taskVersionScheduleConf({ versionId: id }).then((res: any) => {
            if (res.code == 1) {
                this.setState({
    ...
    diffParams = (target: any, haveCode: any) => {
        this.setState({
            showDiff: true,
            compareTo: target,
            haveCode: haveCode
    ...
    renderTaskOperation = (taskInfo: any, record: any) => {
