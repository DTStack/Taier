...
class TaskInfo extends React.Component<any, any> {
...
class DiffTask extends React.Component<any, any> {
    constructor (props: any) {
        super(props);
        this.state = {
            historyParse: {},
    ...
    parseScheduleConf = (data: any, type: any) => {
        const parseScheduleConf: any = {};
        ...
        switch (scheduleConf.periodType) {
            ...
            case '3':
                ...
                let weekDay = scheduleConf.weekDay ? scheduleConf.weekDay.split ? scheduleConf.weekDay.split(',').map((v: any) => {
        ...
        if (type === 1) { // 历史版本
            const upstreamTask = _.map(data.dependencyTasks, (v: any) => {
                return {
                    name: v.taskName,
            ...
            const upstreamTask = _.map(data.taskVOS, (v: any) => {
                return {
                    name: v.name,
                    projectName: v.projectName,
                    tenantName: v.tenantName
    ...
    contrastData = (historyValue: any) => {
        ...
        const currentAttributes: any = ['scheduleStatus', 'effectiveDate', 'isFailRetry', 'schedulingCycle'];

        currentAttributes.forEach((v: any) => {
