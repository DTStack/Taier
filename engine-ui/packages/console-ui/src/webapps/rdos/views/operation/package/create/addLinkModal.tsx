...
class AddLinkModal extends React.Component<any, any> {
    ...
    onOk () {
        ...
        const steps: any = [publishType.RESOURCE, publishType.FUNCTION, publishType.TABLE];
        setTimeout(
            () => {
    ...
    initSelect (rows: any, data: any) {
        let tableKeys: any = []; let resourceKeys: any = []; let functionKeys: any = [];
                    ...
                    <Checkbox
                        ...
                        onChange={(e: any) => {
                            this.setState({
                                selectList: {
                                    ...selectList,
                                    [key]: e.target.checked ? dataList[key].map((item: any) => item.id) : []
                                }
                            })
                        }}>All</Checkbox>
                </Col>
                {<Checkbox.Group value={selectList[key]} onChange={(checkedValue: any) => {
                    this.setState({
                        selectList: {
                            ...selectList,
                            [key]: checkedValue
    ...
    getCount (type: any) {
        ...
        const listKeys = list.map((item: any) => { return item.id });
        let count = 0;
        selectListKeys.map(
            (key: any) => {
