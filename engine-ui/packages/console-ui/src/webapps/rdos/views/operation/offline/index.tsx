...
class OfflineStatistics extends React.Component<any, any> {
    state: any = {
        offline: '',
        topTiming: [],
        topError: [],
    ...
    getTopTaskTime = (dateTime: any) => {
        ...
        Api.getJobTopTime(handleTiming && params).then((res: any) => {
            if (res.code === 1) {
    ...
    getTopJobError = () => {
        const ctx = this
        Api.getJobTopError().then((res: any) => {
            if (res.code === 1) {
    ...
    changeHandleTiming = (value: any) => {
        this.setState({ handleTiming: value }, () => {
            this.getTopTaskTime()
        })
    }

    jumpToOffline (text: any, date: any, status: any) {
        hashHistory.push({
