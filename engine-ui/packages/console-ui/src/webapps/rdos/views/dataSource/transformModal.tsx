...
const formItemLayoutWithOutLabel: any = {
    labelCol: { span: 0 },
    wrapperCol: { span: 24 }
};

class TransformModal extends React.Component<any, any> {
    state: any = {
        nameRule: [{ id: 1 }],
        columnRule: [{ id: 1 }],
        typeRule: [{ id: 1 }]
	...
	UNSAFE_componentWillReceiveProps(nextProps: any) {
        if (nextProps.visible && !this.props.visible) {
            let nameRule: any = [...this.state.nameRule];

            let columnRule: any = [...this.state.columnRule];

            let typeRule: any = [...this.state.typeRule];

            if (nextProps.transformFields.length) {
                nextProps.transformFields.forEach((item: any) => {
                    switch (item.convertObject) {
                        case 1:
                            nameRule.filter(
                                (rule: any) => rule.id == item.id
                            )[0].left = item.convertSrc;
                            nameRule.filter(
                                (rule: any) => rule.id == item.id
                            )[0].right = item.convertDest;
                            break;
                        case 2:
                            columnRule.filter(
                                (rule: any) => rule.id == item.id
                ...
                this.setState({
                    nameRule:
                        nameRule.length > 1
                            ? nameRule.filter((item: any) => item.left && item.right)
                            : nameRule,
                    columnRule:
                        columnRule.length > 1
                            ? columnRule.filter((item: any) => item.left && item.right)
                            : columnRule,
                    typeRule:
                        typeRule.length > 1
                            ? typeRule.filter((item: any) => item.left && item.right)
                            : typeRule
                });
            } else {
                this.setState({
    ...
    // 增加设置
    add = (type: any, id: any) => {
        let data: any = [...this.state[type]];

        let ids = data.map((item: any) => item.id);

        let newId = Math.max(...ids) + 1;

        let target = data.filter((item: any) => item.id == id)[0];
    ...
    // 选择为空时重置对应item
    handleSelect = (target: any, value: any) => {
    ...
    renderFormItem = (data: any, type: any) => {
        const { form } = this.props;
                        ...
                        >
                            {getFieldDecorator(`${type}-${item.id}-left`, {
                                ...
                                type === 'typeRule' ? (
                                    ...
                                    >
                                        {originTypeTransformRule.map((item: any) => {
                                            return (
                                                <Option key={item}>
                                                    {item}
                                                </Option>
        ...
        return (
            <Modal
                ...
                footer={[
                    ...
                    <Button key="save" type="primary" onClick={this.saveConfig}>
                        保存
                    </Button>
                ]}
            >
...
const TransformModalWrapper = Form.create<any>()(TransformModal);
