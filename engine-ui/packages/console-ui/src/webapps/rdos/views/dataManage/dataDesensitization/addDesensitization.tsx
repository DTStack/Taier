...
class AddDesensitization extends React.Component<any, any> {
    ...
    getDesRulesList = (params: any) => {
        ...
        ajax.getDesRulesList(params).then((res: any) => {
            if (res.code === 1) {
                this.setState({
                    rulesList: res.data
                })
    ...
    tableListOption () {
        ...
        return tableList.map((item: any, index: any) => {
    ...
    getColumnsList = (value: any) => {
        ...
        if (projectId && value) {
            ajax.getColumnsList({ tableId: value, projectId: projectId }).then((res: any) => {
                if (res.code === 1) {
                    this.setState({
    ...
    columnsOption () {
        ...
        return columnsList.map((item: any, index: any) => {
            return <Option
                key={item.key}
                value={item.key}
            >
    ...
    rulesOption () {
        ...
        return rulesList.map((item: any, index: any) => {
            return <Option
                key={item.id}
                value={`${item.id}`}
            >
     ...
     */
    isPassConfig = (sampleDataArr: any, beginPos: any, endPos: any) => {
     ...
     */
    setNewReplaceData = (sampleDataArr: any, beginPos: any, endPos: any, newStr: any) => {
...
export default Form.create<any>()(AddDesensitization);
