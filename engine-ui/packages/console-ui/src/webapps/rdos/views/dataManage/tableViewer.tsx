...
export default class TableViewer extends React.Component<any, any> {
    ...
    changeMark () {
        ...
        if (isMark) {
            ajax.cancelMark(this.queryParams).then((res: any) => {
                if (res.code === 1) {
                    message.info('取消收藏成功')
                    this.setState({
                        isMark: !isMark
            ...
            ajax.addMark(this.queryParams).then((res: any) => {
                if (res.code === 1) {
                    message.info('收藏成功')
                    this.setState({
                        isMark: !isMark
    ...
    getTable () {
        ajax.getTable(this.queryParams).then((res: any) => {
            if (res.code === 1) {
    ...
    getCreateCode () {
        !this.state.code ? ajax.getCreateTableCode(this.queryParams).then((res: any) => {
            if (res.code === 1 && res.data) {
                this.setState({
                    visible: true,
                    code: res.data
                        ...
                        <div className="m-tabs m-card bd">
                            ...
                            >
                                ...
                                <TabPane tab="数据预览" key="3">
                                    <div className="box">
                                        {previewData ? <Table
                                            columns={this.previewCols.map((str: any, i: any) => ({
                                                title: str,
                                                dataIndex: str,
                                                key: str + i,
                                                width: '200px'
