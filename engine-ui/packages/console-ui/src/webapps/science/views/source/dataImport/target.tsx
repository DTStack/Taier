...
import { DdlIdePlaceholder } from '../../../comm/DDLCommon'
import { getTableList } from '../../../actions/sourceActions';
...
class ImportTarget extends React.Component<any, any> {
    ...
    tableInput = (tableName: any) => {
        ...
        if (tableName.length > 0) {
            API.getTablesByName({ tableName }).then((res: any) => {
                if (res.code === 1) {
                    changeStatus({
                        tableList: res.data || []
                    });
    ...
    createTable = () => {
        ...
        API.createTableByDDL({ sql: sqlText }).then((res: any) => {
            if (res.code === 1) {
                this.setState({
                    visible: false,
                    tableList: [res.data]
        ...
        return (
            <div style={{ display: display === 'target' ? 'block' : 'none' }}>
                ...
                <Modal className="m-codemodal"
                    title={(
                        <span>建表语句<CopyIcon title="复制模版" style={{ marginLeft: '8px' }} copyText={DdlIdePlaceholder} /></span>
                    )}
                    maskClosable={false}
                    style={{ height: 424 }}
