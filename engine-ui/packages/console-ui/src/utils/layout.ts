...
export function getGeoByRelativeNode(relativeNode: any, node: any) {
	const getX = function (currentNode: any) {
		if (currentNode.index === 1 && currentNode.count === 1) {
			...
			let rowWidth =
				currentNode.count * currentNode.width +
				(currentNode.count - 1) * currentNode.margin;
			const boundX =
				relativeNode.x + Math.round(relativeNode.width / 2) - Math.round(rowWidth / 2);
			if (currentNode.index === 1) {
				return boundX;
			}
			return (
				boundX +
				(currentNode.index - 1) * currentNode.width +
				(currentNode.index - 1) * currentNode.margin
			);
		}
	};
