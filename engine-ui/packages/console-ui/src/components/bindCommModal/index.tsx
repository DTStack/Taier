...
const CustomModal: React.FC = (props: any) => {
	...
	const getServiceParam = () => {
		...
		validateFields((err: any) => {
			if (!err) {
				...
				// 切换队列覆盖默认值name
				if (!isBindTenant)
					params.reqParams = Object.assign(reqParams, { tenantId: tenantInfo.tenantId });
				if (isBindNamespace) {
	...
	const getEnginName = () => {
		...
		for (const key in ENGINE_TYPE) {
			if (
				ENGINE_TYPE[key] !== ENGINE_TYPE.KUBERNETES &&
				ENGINE_TYPE[key] !== ENGINE_TYPE.HADOOP
			) {
		...
		>
			<React.Fragment>
				...
				<Form>
							...
							>
								{tenantList &&
									tenantList.map((tenantItem: any) => {
										return (
											<Option
												key={`${tenantItem.dtUicTenantId}`}
												value={`${tenantItem.dtUicTenantId}`}
					...
					<Form.Item label="集群" {...formItemLayout}>
					...
					{env[ENGINE_TYPE.KUBERNETES] && (
						<div className="border-item">
							...
							<Form.Item label="Namespace" {...formItemLayout}>
