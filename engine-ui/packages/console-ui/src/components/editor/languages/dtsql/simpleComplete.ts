...
function functionsCompleteItemCreater(functions: any) {
	return functions.map((functionName: any, index: any) => {
		return {
			label: functionName,
...
function customCompletionItemsCreater(_customCompletionItems: any) {
	...
	return _customCompletionItems.map(([name, detail, sortIndex, type]: any, index: any) => {
...
function createDependencyProposals() {
	if (!cacheKeyWords.length) {
		...
		cacheKeyWords = keywordsCompleteItemCreater(keywords).concat(
			functionsCompleteItemCreater(functions),
		);
	}
		...
		return new Promise<any>(async (resolve: any, reject: any) => {
			...
			if (completeProvideFunc) {
				...
				if (
					...
					columnContext = autoComplete.suggestColumns.tables.map((table: any) => {
