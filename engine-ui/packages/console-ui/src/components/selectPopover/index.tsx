...
import { Button, Popover, Checkbox, Row, Col, Radio } from 'antd';
import {
	SOURCE_COMPONENTS,
	STORE_COMPONENTS,
	COMPUTE_COMPONENTS,
	...
	componentBtn = () => {
		...
		const popoverFooter = (
			<div style={{ marginTop: 37, float: 'right' }}>
				<Row>
					<Col>
						<Button size="small" onClick={handleCanclePopover}>
							取消
						</Button>
						<Button
							size="small"
		...
		switch (compTypeKey) {
			case TABS_TITLE_KEY.COMMON:
				content = (
					<CheckboxGroup
						style={{ width: '100%' }}
						onChange={this.setCheckboxCompData}
						defaultValue={defaultValue}
					>
						<Row>
							{COMMON_COMPONENTS.map((item: any) => {
								return (
									<Col key={`${item.componentTypeCode}`}>
										<Checkbox value={item.componentTypeCode}>
											{item.componentName}
										</Checkbox>
									</Col>
								);
			...
			case TABS_TITLE_KEY.SOURCE:
				content = (
					<RadioGroup
						style={{ width: '100%' }}
						onChange={this.setRadioCompData}
					...
					>
						<Row>
							{SOURCE_COMPONENTS.map((item: any) => {
								return (
									<Col key={`${item.componentTypeCode}`}>
										<Radio value={item.componentTypeCode}>
											{item.componentName}
										</Radio>
									</Col>
								);
		...
		return (
			...
			>
				<Button
					...
					onClick={() => setPopverVisible()}
				>
					<i className="iconfont icon-zujianpeizhi" style={{ marginRight: 2 }}></i>
					组件配置
				</Button>
			</Popover>
		);
