...
class AddCommModal extends React.Component<any, any> {
	...
	onCheckAllChange = (e: any) => {
		...
		this.setState({
			checkedList: e.target.checked
				? isAddComp
					? this.exChangeCheckedListData(notAddComponetVal)
					: this.exChangeCheckedListData(HADOOP_GROUP_VALUE)
				: isAddComp
				? []
				: defaultCheckedValue,
	...
	getServerParams = (value: any) => {
		...
		const isHadoop =
			hadoopOption === ENGINE_TYPE_NAME.HADOOP || libraOption === ENGINE_TYPE_NAME.HADOOP; // 添加了hadoop
		const isLibra =
			hadoopOption === ENGINE_TYPE_NAME.LIBRA || libraOption === ENGINE_TYPE_NAME.LIBRA; // 添加了libra
	...
	filterExistData = () => {
		...
		return copyOptVal.filter(
			(obj: any) =>
				!hadoopComponentData.some((item: any) => item.componentTypeCode == obj.value),
		);
	};
	renderDiffentEngine = (flag: any) => {
		...
		return flag ? (
			...
			<React.Fragment>
				<FormItem label="必选组件" {...formItemLayout}>
	...
	render() {
		...
		return (
			<Modal
				...
				onCancel={onCancel}
				onOk={() => {
					onOk(isAddCluster ? this.getAddClusterParams() : this.getEngineAndCompParams());
				}}
			>
				<Form>
					{isAddCluster && (
						<FormItem label="集群标识" {...formItemLayout}>
					...
					{!isAddComp && (
						<FormItem label="引擎类型" {...formItemLayout}>
							...
							{isAddCluster && !isAddMultiple && (
								<a className="engine-opera" onClick={this.addEngine}>
									添加引擎
								</a>
							)}
						</FormItem>
					...
					{isAddMultiple ? (
						<React.Fragment>
							...
							<FormItem label="引擎类型" {...formItemLayout}>
								...
								{isAddCluster && isAddMultiple && (
									<a className="engine-opera" onClick={this.delEngine}>
										删除引擎
									</a>
								)}
							</FormItem>
