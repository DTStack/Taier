	...
	const getServiceParam = () => {
		...
		validateFields((err: any, value) => {
			if (!err) {
				...
				setLoading(true);
				Api.switchQueue({ queueId: value?.queueId, tenantId, taskTypeResourceJson })
					.then((res: any) => {
						if (res.code === 1) {
	...
	const actionDom = () => {
		return (
			<div className="o-div--actionDom" onClick={addTaskType}>
				<Icon className="o-icon--actionDom" type="plus-circle" />
				添加资源限制
			</div>
		);
	};
	...
	const removeType = (type) => {
		...
		if (typeof type !== 'undefined') {
			...
			deleteIndex !== -1 &&
				setTypeList({
					...typeList,
					union: [...union.filter((numType) => numType !== type)],
				});
		}
	};
