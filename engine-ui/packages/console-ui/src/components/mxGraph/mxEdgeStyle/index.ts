...
const MyEdgeStyle = function (Mx: any) {
	const { mxConstants, mxEdgeStyle, mxPoint, mxUtils, mxStyleRegistry, mxRectangle } = Mx;
	...
	return {
		...
		OrthConnector: function (state: any, source: any, target: any, points: any, result: any) {
			...
			var scaledSourceBuffer =
				state.view.scale * mxEdgeStyle.getJettySize(state, source, target, points, true);
			var scaledTargetBuffer =
				state.view.scale * mxEdgeStyle.getJettySize(state, source, target, points, false);
			...
			if (
				tooShort ||
				(mxEdgeStyle.orthPointsFallback && points != null && points.length > 0) ||
				sourceEdge ||
				targetEdge
			) {
			...
			var lastOrientation =
				(dir[0] & (mxConstants.DIRECTION_MASK_EAST | mxConstants.DIRECTION_MASK_WEST)) > 0
					? 0
					: 1;
