 ...
 * create: 2019/10/28
 */
public class OrderLinkedBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {
...
//    private final ReentrantLock putLock = new ReentrantLock();
    ...
    public boolean contains(Object o) {
        ...
        allLock.lock();
        try {
            String oldJobId = ((BatchJobElement) o).getJobId();
            for (Node<E> p = head.next; p != null; p = p.next) {
                if (oldJobId.equals(((BatchJobElement) p.item).getJobId())) {
                    return true;
                }
            }
            return false;
    ...
    public boolean remove(String jobId) {
        ...
        try {
            ...
            for (Node<E> trail = head, p = trail.next;
                 p != null;
                 trail = p, p = p.next) {
                BatchJobElement oo = (BatchJobElement) p.item;
                if (jobId.equals(oo.getJobId())) {
                    unlink(p);
                    return true;
                }
            }
    ...
    public IndexNode<E> getElement(String jobId) {
        if (StringUtils.isBlank(jobId)) {
            return null;
        }
        try {
