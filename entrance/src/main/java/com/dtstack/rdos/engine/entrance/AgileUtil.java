...
public class AgileUtil {
    ...
    private static String HADOOP_CONF_DIR = System.getProperty("user.dir") + "/etc/hadoop";
    private static String CONFIG = System.getProperty("user.dir") + "/hadoop_pkg/agile.yml";
    ...
    private final static int HA_NODE_LIMIT = 3;
    ...
    private static void postHadoopConfDir() {
        File hadoopConfDir = new File(HADOOP_CONF_DIR);
    ...
    private static void generateHadoopConfFile(String fileName, Node nn1, Node nn2, String zkIps, String[] hiveMetaStoreInfo) throws Exception {
        ...
        if (defaultClusterFile.exists()) {
            ...
            File newFile = new File(HADOOP_CONF_DIR + "/" + fileName);
            LOGGER.info("创建文件：" + newFile.getPath());
            if (newFile.createNewFile()) {
                Files.write(newFile.toPath(), paramReplace(yarnSite.toString(), nn1, nn2, zkIps, hiveMetaStoreInfo).getBytes());
            } else {
                throw new RuntimeException(newFile.getName() + "创建失败");
            }
        }
    }
