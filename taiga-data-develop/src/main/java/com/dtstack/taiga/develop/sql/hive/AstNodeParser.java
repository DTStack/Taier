package com.dtstack.taiga.develop.sql.hive;

import com.dtstack.taiga.develop.sql.*;
import com.dtstack.taiga.develop.sql.calcite.LineageParser;
import com.dtstack.taiga.develop.sql.handler.IUglySqlHandler;
import com.dtstack.taiga.develop.sql.hive.node.NodeParser;
import com.dtstack.taiga.develop.sql.hive.node.OtherNodeParser;
import com.dtstack.taiga.develop.sql.node.*;
import com.dtstack.taiga.develop.sql.utils.SqlFormatUtil;
import com.dtstack.taiga.develop.sql.utils.SqlRegexUtil;
import com.dtstack.taiga.develop.sql.utils.SqlTypeRegexUtil;
...
public class AstNodeParser extends BaseSqlParser {
    ...
    private ParseResult getParseResult(String originSql, String currentDb, Map<String, List<Column>> tableColumnsMap, ParseResult parseResult, ASTNode root) {
        ...
        com.dtstack.taiga.develop.sql.node.Node node = nodeParser.parseSql((ASTNode) root.getChild(0), currentDb, tableColumnsMap, new HashMap<>());
    ...
    public List<Table> parseTables(String currentDb, String sql) throws Exception {
        ...
        com.dtstack.taiga.develop.sql.node.Node node = nodeParser.parseSql((ASTNode) root.getChild(0), currentDb, new HashMap<>(), new HashMap<>());
        if (Objects.isNull(node)) {
            return new ArrayList<>();
        }
        nodeParser.parseSqlTable(node, tables);
    ...
    private List<ColumnLineage> getColumnLineages(com.dtstack.taiga.develop.sql.node.Node sqlNode, LineageParser lineageParser) {
     ...
     * @return
     */
    private List<TableLineage> getTableLineages(com.dtstack.taiga.develop.sql.node.Node sqlNode, LineageParser lineageParser) {
    ...
    /*
       获取main主表
    */
    private void getMainTable(com.dtstack.taiga.develop.sql.node.Node node, ParseResult parseResult, ASTNode root) {
