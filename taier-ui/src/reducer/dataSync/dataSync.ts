...
import { RDB_TYPE_ARRAY } from '@/constant';
	...
	switch (action.type) {
		...
		case sourceMapAction.DATA_SOURCE_CHANGE: {
			...
			if (typeof key !== 'undefined') {
				for (const i in clone.sourceList) {
					const source = clone.sourceList[i];
					if (source.key === key) {
						clone.sourceList[i] = {
							name: dataName,
							sourceId: id,
...
export const targetMap = (state: any = {}, action: any) => {
	switch (action.type) {
		...
		case targetMapAction.DATA_TARGETMAP_CHANGE: {
			...
			if (writeMode) clone.type.writeMode = writeMode;
			if (typeof extralConfig !== 'undefined') {
				clone.extralConfig = extralConfig;
			}
	...
	switch (action.type) {
		...
		case keyMapAction.SET_NAME_MAP: {
			...
			sourceCol.forEach((o: any, i: any) => {
				...
				const idx = targetCol.findIndex((o: any) => {
...
const setting = (
	...
	switch (action.type) {
		...
		case settingAction.INIT_CHANNEL_SETTING: {
			return initialChannelSettingState;
		}
