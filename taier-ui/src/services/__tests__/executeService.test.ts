...
describe('Test ExecuteService', () => {
	...
	it('Should execSql failed without taskType', () => {
		...
		executeService.execSql(1, { id: 1 } as CatalogueDataProps & IOfflineTaskProps, {}, [
			'show tables',
		]);
	...
	it('Should execSql failed since request failed', async () => {
		...
		await waitFor(() => {
			...
			expect((executeService.emit as jest.Mock).mock.calls[1]).toEqual([
				EXECUTE_EVENT.onEndRun,
				1,
			]);
		});
	});
	...
	it('Should execSql successful without jobId', async () => {
		...
		expect((taskResultService.setResult as jest.Mock).mock.calls[0]).toEqual([
			`1-${md5('test')}`,
			[],
		]);
