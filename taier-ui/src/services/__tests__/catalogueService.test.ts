...
describe('Test catalogue service', () => {
	...
	it('Should load root folder', async () => {
		...
		await new Promise<void>((resolve) => {
			setTimeout(() => {
				expect(
					(resourceManagerService.add as jest.Mock).mock.calls[0][0],
				).toMatchSnapshot();

				expect(
					(functionManagerService.add as jest.Mock).mock.calls[0][0],
				).toMatchSnapshot();
	...
	it('Should load tree node', async () => {
		...
		await new Promise<void>((resolve) => {
			setTimeout(() => {
				expect(
					(molecule.folderTree.update as jest.Mock).mock.calls[0][0],
				).toMatchSnapshot();
				resolve();
			}, 0);
		});
