...
import { Modal, Row, message, Col, Tree, Checkbox } from 'antd';
...
class RestartModal extends React.Component<any, any> {
	...
	onCheck = (checkedKeys: any) => {
		...
		this.setState({
			...
			isAllChecked: isEqualArr(allSelectedData, hasCheckedVal),
		});
	};
	...
	handleAllSelect = (data: any[]) => {
		...
		// 取消全选置空， 不能使用全选的缓存数据
		this.setState({
			checkedKeys: data.length === 0 ? [] : this.cacheAllSelected,
		});
	};
			...
			const nodes = data.map((item: any) => {
				...
				const content = (
					<Row>
						...
						<Col span={6}>
							<span>{taskTypeText(taskType)}</span>
						</Col>
					</Row>
				);
		...
		return (
			...
			>
				<Row>
					<Col span={12}>
						请选择要重跑的任务: &nbsp;&nbsp;
						<Checkbox onChange={this.handleCheckboxChange} checked={isAllChecked}>
							全选
						</Checkbox>
					</Col>
					<Col span={12} className="txt-right">
						业务日期：{restartNode ? restartNode.businessDate : ''}
					</Col>
				</Row>
