...
function HistoryLog (props: HistoryLogProps) {
    ...
    function getTableData (page = 1, pageSize = 20, sort?: string, orderBy?: React.Key) {
        Api.getHistoryLog({ taskId: props.id, currentPage: page, pageSize, sort, orderBy }).then((res: any) => {
            if (res.code === 1) {
    ...
    function sorter (pagination: TablePaginationConfig, filters: Record<string, FilterValue | null>, sorter:  SorterResult<any> | SorterResult<any>[] ) {
    ...
    const columns: ColumnsType<IDataSource>  = [
        ...
        },
        {
            ...
            width: '140px',
            render: (text, record) => {
                const href = req.DOWNLOAD_HISTORY_LOG + '?applicationId=' + record.applicationId + '&projectId=' + '' + '&jobId=' + jobId;
                return <a download href={href}>下载</a>
            }
        }
    ]
    ...
    const pagination = () => <Pagination
        ...
        showSizeChanger
        showTotal={total => <>共<span style={{ color: '#3F87FF' }}>{total}</span>条数据</>}
        onChange={getTableData}
        onShowSizeChange={getTableData}
    />

    return <div className="dt-table-fixed-contain-footer" style={{ padding: '0 20px' }}>
        <Table
