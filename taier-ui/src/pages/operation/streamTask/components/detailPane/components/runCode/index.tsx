...
const Api = {
	getRealtimeTaskTypes: () =>
		Promise.resolve({
			...
			message: null,
			data: [
				{ key: 0, value: 'FlinkSQL', left: 0, right: 'FlinkSQL' },
				{ key: 1, value: 'Flink', left: 1, right: 'Flink' },
				{ key: 11, value: '实时采集', left: 11, right: '实时采集' },
			],
			space: 0,
			version: null,
			success: true,
							...
							<Input
								...
								value={
									resourceList && resourceList.length
										? resourceList[0].resourceName
										: ''
								}
							/>
						</FormItem>
						<FormItem {...formItemLayout} label="mainClass">
							<Input disabled value={mainClass} />
						</FormItem>
						<FormItem {...formItemLayout} label="命令行参数">
							<Input disabled value={exeArgs} />
						</FormItem>
						<FormItem {...formItemLayout} label="附加资源">
							<Select
								disabled
								mode="multiple"
								value={
									Array.isArray(additionalResourceList) &&
									additionalResourceList.map((item: any) => item.resourceName)
								}
							/>
						</FormItem>
	...
	render() {
		...
		return (
			<div className="m-tabs" style={{ height: '100%' }}>
				...
				{tabKey === 'source' && (
					<div style={editorBoxStyle}>{this.getEditor(sourceParams!)}</div>
				)}
				{tabKey === 'sink' && (
					<div style={editorBoxStyle}>{this.getEditor(sinkParams!)}</div>
				)}
				{tabKey === 'side' && (
					<div style={editorBoxStyle}>{this.getEditor(sideParams!)}</div>
				)}
