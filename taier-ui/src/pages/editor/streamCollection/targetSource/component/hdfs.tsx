...
export default (props: { collectionData: any }) => {
    ...
    return (
        <React.Fragment>
            <FormItem label="路径" name="path" rules={[{ required: true }]}>
                <Input placeholder="例如: /app/batch" />
            </FormItem>
            <FormItem label="写入分区目录名称" name="fileName">
                <Input placeholder="写入分区目录名称为空，则默认在当前路径下写入" />
            </FormItem>
            <FormItem label="文件类型" name="fileType" rules={[{ required: true }]}>
                <Radio.Group>
                    <Radio value="orc">orc</Radio>
                    <Radio value="text">text</Radio>
                    <Radio value="parquet">parquet</Radio>
                </Radio.Group>
            </FormItem>
            {isText && (
                <React.Fragment>
                    <FormItem label="列分隔符" name="fieldDelimiter" tooltip={splitCharacter}>
                        {/* eslint-disable-next-line no-octal-escape */}
                        <Input placeholder="例如: 目标为hive则 分隔符为\001" />
                    </FormItem>
                    <FormItem label="编码" name="encoding" rules={[{ required: true }]}>
                        <Select>
            ...
            <FormItem label="写入模式" className="txt-left" name="writeMode" rules={[{ required: true }]}>
                <Radio.Group>
