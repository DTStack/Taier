...
export default (props: { collectionData: any; sourceList: IDataSourceUsedInSyncProps[] }) => {
    ...
    return (
        <React.Fragment>
            ...
            <FormItem name="message" label="测试信息" rules={[{ required: false, message: '请输入测试信息' }]}>
                <Input.TextArea
                    rows={3}
                    placeholder={`测试信息将在连接建立后发送一次给主机，用作连接测试；内容为空时不进行发送`}
                    style={{ width: '100%' }}
                />
            </FormItem>
            <FormItem
            ...
            <FormItem name="collectPoint" label="采集起点">
                <Radio value="taskRun" disabled checked>
                    从任务运行时开始
                </Radio>
            </FormItem>
        </React.Fragment>
