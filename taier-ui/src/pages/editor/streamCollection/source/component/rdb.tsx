...
export default (props: { collectionData: any; sourceList: IDataSourceUsedInSyncProps[] }) => {
	...
	const sourceDataOptions = sourceList?.filter(d => d.dataTypeCode === sourceMap?.type).map?.((o: IDataSourceUsedInSyncProps) => {
		return <Option key={o.dataInfoId}
					   value={o.dataInfoId}>{o.dataName}{DATA_SOURCE_VERSION[o.dataTypeCode] && ` (${DATA_SOURCE_VERSION[o.dataTypeCode]})`}</Option>
	})
	...
	return <React.Fragment>
		...
		{rdbmsDaType === SYNC_TYPE.INTERVAL && <React.Fragment>
			...
			>
				<InputNumber style={{width: 'calc(100% - 20px)', marginRight: 8}} min={5} max={3600} step={1}
							 addonAfter='秒'/>
			</FormItem>
			<FormItem
				name="extralConfig"
				...
				<FormItem wrapperCol={{
					...
					{distributeTable && distributeTable.map((table: any, index: any) => {
						...
						return (<div className="distribute-table-box" key={`${index}`}>
							<Card className="distribute-table-box__ant-card" title={null}>
								<FormItem
									label={
										<span>分组名&nbsp;
											<Tooltip
												title={'分组名将参与写入目标名称的拼接，例如：写入Hive表时，分组名将作为Hive表名的一部分'}>
							...
							{couldEdit && <DeleteOutlined className="distribute-table-box__icon-delete"
