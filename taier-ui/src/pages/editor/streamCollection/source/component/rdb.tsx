...
export default (props: { collectionData: any; sourceList: IDataSourceUsedInSyncProps[] }) => {
        ...
        switch (collectTypeValue) {
            ...
            case COLLECT_TYPE.TIME: {
                return (
                    ...
                    >
                        <DatePicker showTime placeholder="请选择起始时间" format="YYYY-MM-DD HH:mm:ss" />
                    </FormItem>
                );
            }
    ...
    const renderSlot = (isPostgre: boolean) => {
        ...
        return (
            <>
                <FormItem name="slotConfig" label="Slot配置" rules={[{ required: true, message: '请选择Slot配置' }]}>
                ...
                {slotConfig === Number(Object.keys(SLOAR_CONFIG_TYPE)[0]) ? (
                    <FormItem name="slotName" label="Slot" rules={[{ required: true, message: '请选择Slot' }]}>
                        <Select
                            getPopupContainer={(triggerNode: any) => triggerNode}
                            disabled={isEdit}
                            style={{ width: '100%' }}
    ...
    const renderTransferType = (type: number, isEdit: boolean) => {
        return (
            <FormItem name="pavingData" label="格式转换" valuePropName="checked" tooltip={sourceFormat}>
    ...
    return (
        <React.Fragment>
            ...
            {isOracle && showPDB && (
                <FormItem name="pdbName" label="PDB" rules={[{ required: isOracle && showPDB, message: '请选择PDB' }]}>
                    <Select
                        getPopupContainer={(triggerNode: any) => triggerNode}
                        disabled={isEdit}
                        placeholder="请选择PDB"
            ...
            {rdbmsDaType === SYNC_TYPE.INTERVAL && (
                <React.Fragment>
                    <FormItem name="tableName" label="表" rules={[{ required: true, message: '请选择表' }]}>
                        <Select
                            getPopupContainer={(triggerNode: any) => triggerNode}
                            disabled={isEdit}
                            style={{ width: '100%' }}
                    ...
                    <FormItem name="startLocation" label="采集起点" tooltip={startLocation}>
                        <Input />
                    </FormItem>
                    <FormItem
                    ...
                    <FormItem name="extralConfig" label="高级配置" tooltip={extralConfig}>
            ...
            {rdbmsDaType !== SYNC_TYPE.INTERVAL && (
                <React.Fragment>
                    {!isSqlServer(type) && type !== DATA_SOURCE_ENUM.ORACLE && (
                        <FormItem name="multipleTable" label="是否分表" tooltip={multipleTableTip}>
                            <Radio.Group>
                                <Radio value={false}>不分表</Radio>
                                <Radio value>分表</Radio>
                            </Radio.Group>
                        </FormItem>
                    )}
                    {multipleTable && (
                                                ...
                                                <FormItem
                                                    label={
                                                        <span>
                                                            ...
                                                            <Tooltip
                                                                title={
                                                                    '分组名将参与写入目标名称的拼接，例如：写入Hive表时，分组名将作为Hive表名的一部分'
                                                                }
                                                            >
                    ...
                    {!multipleTable && (
                        <FormItem name="table" label="表" rules={[{ required: true, message: '请选择表' }]}>
                            <Select
                                getPopupContainer={(triggerNode: any) => triggerNode}
                                mode="multiple"
                                style={{ width: '100%' }}
                    ...
                    <FormItem name="cat" label="数据操作" rules={[{ required: true, message: '请选择数据操作' }]}>
                        <Checkbox.Group
                            options={[
