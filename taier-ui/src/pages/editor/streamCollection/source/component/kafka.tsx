...
export default (props: { collectionData: any; sourceList: IDataSourceUsedInSyncProps[]; form: FormInstance }) => {
	...
	const sourceDataOptions = sourceList?.map?.((o: any) => {
		return <Option key={o.id}
					   value={o.id}>{o.name}{DATA_SOURCE_VERSION[o.type as DATA_SOURCE_ENUM] && ` (${DATA_SOURCE_VERSION[o.type as DATA_SOURCE_ENUM]})`}</Option>
	})
	return <React.Fragment>
		...
		<FormItem
			name="codec"
			label="读取类型"
			rules={[{required: true, message: '请选择读取类型'}]}
		>
			<Select
				getPopupContainer={(triggerNode: any) => triggerNode}
		...
		{[DATA_SOURCE_ENUM.KAFKA_2X, DATA_SOURCE_ENUM.KAFKA, DATA_SOURCE_ENUM.TBDS_KAFKA, DATA_SOURCE_ENUM.KAFKA_HUAWEI].includes(type) &&
			<React.Fragment>
				<FormItem
					required
					label="Offset"
