...
export default function FlinkResultPanel({ current }: Pick<IEditor, 'current'>) {
	...
	const loadPartitions = async (index: any, sourceId: any, tableName: any) => {
		...
		if (res.code === 1) {
			const data = res.data || {};
			panelColumn[index]['storeType'] =
				data.tableLocationType == 'kudu' ? data.tableLocationType : data.fileType;
			if (data.tableLocationType === 'hive') {
				// hive表显示分区
	...
	const getPartitionLists = (index: any, sourceId: any, tableName: any) => {
		...
		partitionAPI &&
			partitionAPI({ sourceId, tableName }).then((v: any) => {
				if (v.code === 1) {
					...
					panelColumn[index]['havePartitionfields'] =
						partitionOptionTypeList && !!partitionOptionTypeList.length;
				} else {
				...
				if (
					[DATA_SOURCE_ENUM.HIVE, DATA_SOURCE_ENUM.INCEPTOR].includes(
					...
					if (v === 'type') {
						...
						if (isKafka(value)) {
							if (value === DATA_SOURCE_ENUM.KAFKA_CONFLUENT) {
								panelColumn[index]['sinkDataType'] =
						...
						panelColumn[index][v] = isRDB(panelColumn[index]['type'])
							? 1000
							: undefined;
					} else if (v == 'batchSize') {
