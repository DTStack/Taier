...
import { dimensionDefaultValue } from '../flinkHelper';
...
export default function DimensionForm({
	...
	const originOption = (type: any, arrData: any) => {
		switch (type) {
			...
			case 'originType':
				return arrData.map((v: IDataSourceUsedInSyncProps) => {
					return (
	...
	const initCommonElements = () => {
		const elements = haveDataPreview(panelColumn[index].type) ? (
			<Row>
	...
	const originOptionTypes = originOption('originType', originOptionType[data.type] || []);
	...
	const primaryKeyOptionTypes = originOption('primaryType', panelColumn[index].columns || []);
			...
			<Form {...formItemLayout} form={form} initialValues={initialValues}>
				...
				{isES(panelColumn[index].type) ? (
					...
					>
						<Input
							placeholder="请输入索引"
							onChange={(e: any) => handleInputChange('index', index, e.target.value)}
						/>
					</FormItem>
				) : null}
				{initCommonElements()}
				...
				>
					<Input
						placeholder="请输入映射表名"
						onChange={(e: any) => handleInputChange('tableName', index, e.target.value)}
					/>
				</FormItem>
				<Row>
									...
									>
										...
										<span>
											...
											<Popconfirm
												title="确认清空所有字段？"
												onConfirm={() => {
													handleInputChange('deleteAllColumn', index);
												}}
												okText="确认"
												cancelText="取消"
											>
						...
						case DATA_SOURCE_ENUM.UPRedis: {
							return (
								...
								>
									<Input
										placeholder={
											panelColumn[index].type === DATA_SOURCE_ENUM.MONGODB
												? '请输入主键'
												: '维表主键，多个字段用英文逗号隔开'
										}
										onChange={(e: any) =>
				...
				<FormItem name="parallelism" label="并行度">
					<InputNumber
						...
						min={1}
						onChange={(value: any) => handleInputChange('parallelism', index, value)}
					/>
				</FormItem>
				<FormItem
					label="缓存策略"
				...
				>
					...
					>
						<Option
							key="None"
							value="None"
							disabled={isCacheOnlyAll(panelColumn[index].type)}
						>
							None
						</Option>
						<Option
							key="LRU"
							value="LRU"
							disabled={isCacheExceptLRU(panelColumn[index].type)}
						>
							LRU
						</Option>
				...
				<FormItem label="允许错误数据" tooltip={asyncTimeoutNumDoc} name="errorLimit">
					<InputNumber
						...
						placeholder="默认为无限制"
						min={0}
						onChange={(value: any) => handleInputChange('errorLimit', index, value)}
					/>
				</FormItem>
				...
				{haveAsyncPoolSize(panelColumn[index].type) ? (
					<FormItem name="asyncPoolSize" label="异步线程池">
						<Select
							style={{ width: '100%' }}
							onChange={(v: any) => {
								handleInputChange('asyncPoolSize', index, v);
							}}
						>
				...
				{haveCustomParams(panelColumn[index].type) && (
					<CustomParams
