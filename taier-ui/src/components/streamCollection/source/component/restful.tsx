...
const tabList = [{ key: '0', tab: '第一次请求' }, { key: '1', tab: '第二次请求' }];
...
export default (props: { collectionData: any; }) => {
    ...
    const validateUrl = (rule: any, val: any, callback: any) => {
        if (!val) {
    ...
    const renderSelect = (dataSource: any[]) =>
        <Select>
            {dataSource.map(({ text, value }) => <Option value={value} key={value}>{text}</Option>)}
        </Select>
    ...
    return <React.Fragment>
                    ...
                    },
                    {
                        ...
                        render: (text: any, record: any, index: any, onTableCellChange: any, operations: any) => (
                            <div style={{ 'position': 'relative' }}>
                                ...
                                >
                                    {RESTFUL_STRATEGY.map(({ text, value }: any) => <Option key={value} value={value}>{text}</Option>)}
                                </Select>
                                <MinusCircleOutlined className="c-icon-delete" onClick={operations} />
                            </div>)
                    }
        ...
        >
            <Button
                ...
                onClick={loadRestFulPreview}
            >
                数据预览
            </Button>
        </FormItem>
        {restfulPreviewVisable && <FormItem
            key="previewTab"
            label=" "
            className="c-restful-preview"
        >
            <Card
                key="previewCard"
                tabBarExtraContent={
                    <a onClick={() => { setRestfulPreviewVisable(false) }}>
                        <UpOutlined />
                    </a>
                }
                tabList={tabList}
                activeTabKey={previewTabKey}
                onTabChange={(key: any) => { setPreviewTabKey(key) }}
            >
                <Editor
                    sync
