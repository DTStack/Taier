...
export default (props: { collectionData: any; }) => {
    ...
    const haveDataSequence = [DATA_SOURCE_ENUM.KAFKA, DATA_SOURCE_ENUM.KAFKA_2X, DATA_SOURCE_ENUM.TBDS_KAFKA, DATA_SOURCE_ENUM.KAFKA_HUAWEI].includes(targetMap.type);
    ...
    return (<React.Fragment>
        ...
        {haveDataSequence && canPartitionKeyWrite(sourceMap) && <FormItem
            label="Partition Key"
            name="partitionKey"
            tooltip={writePartitionKey}
        >
            <Select
                getPopupContainer={(triggerNode: any) => triggerNode}
                disabled={isEdit}
                ...
                style={{ width: '100%' }}
            >
                {Array.isArray(tableFieldsList) && tableFieldsList.map((item: any) => (
