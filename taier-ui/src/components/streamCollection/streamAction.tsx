...
const _saveTask = async (saveMode?: any, isSilent?: boolean | 'undefined') => {
	...
	if (current?.tab?.data) {
		currentPage.sqlText = current.tab.data.value;
	}
	...
	const isFlinkSQLGuide =
		currentPage.createModel == FLINK_SQL_TYPE.GUIDE || !currentPage.createModel;
	const isFlinkGraphData = currentPage.createModel == FLINK_SQL_TYPE.GRAPH;
	/**
	 * 当目标数据源为Hive时，必须勾选Json平铺
	 */
	if (currentPage.taskType == TASK_TYPE_ENUM.DATA_ACQUISITION) {
		const haveJson =
	 ...
	 * 校验源表和结果表和维表
	 */
	if (currentPage.taskType == TASK_TYPE_ENUM.SQL && isFlinkSQLGuide) {
	...
	if (outputCheckFormParams.length > 0) {
		for (let index = 0, len = outputCheckFormParams.length; index < len; index++) {
			// 检查出一个未填选项,不再检查其它的选项,只弹一次错误
			...
			if (!result.status) {
				return message.error(
					`结果表${outputCheckFormParams[index].props.index + 1} ${
						tableName ? `(${tableName})` : ''
					}: ${result.message || '您还有未填选项'}`,
				);
			}
		}
	}
	if (dimensionCheckFormParams.length > 0) {
		for (let index = 0, len = dimensionCheckFormParams.length; index < len; index++) {
			// 检查出一个未填选项,不再检查其它的选项,只弹一次错误
			...
			if (!result.status) {
				return message.error(
					`维表--维表${dimensionCheckFormParams[index].props.index + 1} ${
						tableName ? `(${tableName})` : ''
					}: ${result.message || '您还有未填选项'}`,
				);
			}
	...
	if (Array.isArray(additionalResourceList)) {
		...
		if (
			additionalResourceList.length > 0 &&
			intersection(additionalResourceList, currentPage.resourceIdList).length > 0
		) {
			message.error('资源与附加资源不可相同');
			return;
		}
		currentPage.additionalResourceIdList = additionalResourceList.map(
	 ...
	 * flink图操作的处理
	 */
	if (currentPage.taskType == TASK_TYPE_ENUM.SQL && isFlinkGraphData) {
	...
	if (showDirtyManage) {
		if (
			streamTaskDirtyDataManageVO?.outputType === DIRTY_DATA_SAVE.BY_MYSQL &&
			!streamTaskDirtyDataManageVO?.linkInfo?.sourceId
		) {
