...
const createLineTo = (isLined = false, onLineChanged = jest.fn(), onLineClick = jest.fn()) => {
...
describe('Test LineTo Class Component', () => {
    ...
    it('Should support remove line', async () => {
        ...
        expect(svg?.querySelector<HTMLDivElement>('.taier__lintTo__tooltip__content')?.style.display).toBe('block');
        jest.useFakeTimers();

        fireEvent.mouseOut(line!);
        jest.advanceTimersByTime(500);
        expect(svg?.querySelector<HTMLDivElement>('.taier__lintTo__tooltip__content')?.getAttribute('style')).toBe(
            null
        );
