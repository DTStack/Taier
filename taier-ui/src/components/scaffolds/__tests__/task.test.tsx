...
describe('Test Task Scaffolds', () => {
	...
	describe('Test SelectWithRequest Component', () => {
		...
		it('Should render error message by Tooltip', async () => {
			(http.get as jest.Mock)
				.mockReset()
				.mockResolvedValue({ code: 0, message: 'error message' });
			await act(async () => {
				render(
					<FormContainer>
						<SelectWithRequest
		...
		it('Should support get create sql', async () => {
			...
			const { container, getByTestId } = render(
				<FormContainer>
					...
					<FormItem name={['sourceMap', 'partition']} initialValue={true} />
			...
			expect(container.querySelector<HTMLInputElement>('input#targetMap_table')!.value).toBe(
				'xxxx',
			);
		});
	});
	...
	describe('Test SelectWithPreviewer Component', () => {
		...
		it('Should support on multiple mode', () => {
			...
			expect(
				container.querySelector('.ant-select-selection-overflow-item'),
			).toBeInTheDocument();
		});
		...
		it('Should support preview data', async () => {
			...
			expect(
				container.querySelector<HTMLSpanElement>('.anticon-fund-view')!.style.pointerEvents,
			).toBe('auto');
	...
	describe('Test ResourcePicker Component', () => {
		it('Should match snapshot', () => {
			expect(
				render(
					<ResourcePicker id="mock" value="1" onChange={jest.fn()} event={{} as any} />,
				).asFragment(),
			).toMatchSnapshot();
		});
	});
		...
		it('Should call request', async () => {
			...
			const { container } = render(
				<FormContainer>
					...
					<FormItem name={['sourceMap', 'isFirstLineHeader']} initialValue={true} />
					<FormItem name={['sourceMap', 'sourceId']} initialValue="1" />
